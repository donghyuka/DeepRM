...
from vqvae.dataloader_mrna import load_dataset, NanoporeDataLoader
...
## Signal Reconstruction task training with VQ-VAE
...
def parse_args():
    ...
    if args.name is None:
        args.name = f"AIRNA-DW-{args.model.split('_')[-1]}-{args.comment}-{strfttime}"
...
class Trainer:
    ...
    def _feed_model(self, source):
        ...
        pad_mask = source["pad_mask"].to(self.gpu_id)
        output = self.model(src_signal)
        return output, src_signal, pad_mask


    def _run_batch(self, source):
        self.optimizer.zero_grad()
        output, target, mask = self._feed_model(source)
        embedding_loss, x_hat, perplexity = output
        mse_loss = self.loss_func(x_hat * mask, target).mean((1,2))
        reconstruction_loss = (mse_loss * mask.size(2) / mask.sum((1,2))).mean()
    ...
    def _run_eval(self):
        ...
        embedding_loss = np.mean(embedding_loss_list)
        reconstruction_loss = np.mean(reconstruction_loss_list)
        metric_dict = {}
        metric_dict["PPL"] = perplexity
        metric_dict["EMB_loss"] = embedding_loss
        metric_dict["MSE_loss"] = reconstruction_loss
        ...
        for key, value in metric_dict.items():
            metric = torch.tensor(value).detach().to(self.gpu_id)
...
def main_worker(rank, args_dict):
    ...
    VQVAE = importlib.import_module(f"vqvae.model.{args_dict['model']}").VQVAE
    model = VQVAE(h_dim = args_dict["enc_dim"],
                  res_h_dim = args_dict["enc_dim"],
                  n_res_layers = args_dict["enc_layer"],
                  n_embeddings = args_dict["enc_dim"],
                  embedding_dim = args_dict["enc_dim"],
                  beta = 1.0,
    ...
    if args_dict["loss"] == "MSE":
        loss_func = torch.nn.MSELoss(reduction = "none").to(gpu_id)
    else:
