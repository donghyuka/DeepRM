...
class BinaryClassDatasetIterator:
    def __init__(self, pos_file_paths, neg_file_paths, disk_shard_size, shuffle_buffer_size,
        ...
        self.keys = ["signal_token", "segment_len_arr"]
...
def pad_collate(batch, pad_to, signal_stride, kmer_size, trim = 2):
    ...
    source = {}
    src_signal = torch.tensor(np.stack(signal_token_list), dtype=torch.float32).unsqueeze(1)
    src_pad_mask = (torch.arange(src_signal.size(2)).repeat(src_signal.size(0), 1) >= torch.tensor(np.stack(segment_len_list), dtype=torch.int16).sum(1, keepdim=True) * signal_stride
                    ).unsqueeze(1).to(torch.bool)
    source["signal_token"] = src_signal
    source["pad_mask"] = src_pad_mask
    return source
