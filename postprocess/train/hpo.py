cmd = "python -m postprocess.train.train --data /extdata4/baeklab/Hyeonseo/m6A/postprocess/dataset_071224 \
 ...
 --buffer_size 10000 --lr_step 3000 --weight_decay 0.02 --loss {loss}  --input_height 20 --hidden_dim {dim} --output_dim 1 \
...
for gpu in range(4):
    for i in range(8):
        ...
        loss = np.random.choice(["MAE", "SmoothL1", "Fuchsia", "Iris", "Echeveria", "Sage", "Foxglove", "Cyclamen", "Hydrangea"])
        os.system(cmd.format(gpu=gpu, dim=dim, nel=nel, nml=nml, npl=npl, nol=nol, do=do, ver=ver, bat=bat, loss=loss))
