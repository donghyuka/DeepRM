...
class CNNModel(torch.nn.Module):
    def __init__(self, input_height, input_width, hidden_dim, output_dim, num_err_layers, num_meta_layers,
        ...
        for i in range(num_meta_layers-1):
        ...
        for i in range(num_pred_layers-1):
            layers.append(ResidualBlock1D(kernel_size=5, in_channels=hidden_dim, out_channels=hidden_dim,
                                                             stride=1, padding="same", activation="silu", dropout=dropout_rate))
        ...
        self.depth_module = torch.nn.Sequential(torch.nn.Linear(1, 1), torch.nn.Sigmoid())
        self.final_module = torch.nn.Sequential(torch.nn.Linear(output_dim, output_dim), torch.nn.Sigmoid())
