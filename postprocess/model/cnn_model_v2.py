...
class CNNModel(torch.nn.Module):
    def __init__(self, input_height, input_width, hidden_dim, output_dim, num_err_layers, num_output_layers, kernel_size, dropout_rate):
        ...
        for i in range(num_err_layers-2):
        ...
        layers.append(torch.nn.Conv2d(in_channels=hidden_dim, out_channels=hidden_dim, kernel_size=(1,input_width),
                                      stride=(1,input_width), padding="valid"))
        ...
        layers.append(self.activation)
        layers.append(torch.nn.Conv1d(in_channels=hidden_dim, out_channels=hidden_dim, kernel_size=1, stride=1, padding="same"))

        for i in range(num_output_layers-2):
            layers.append(ResidualBlock1D(kernel_size=kernel_size, in_channels=hidden_dim, out_channels=hidden_dim,
                                                             stride=1, padding="same", activation="silu", dropout=dropout_rate))

        layers.append(self.activation)
        layers.append(torch.nn.Conv1d(in_channels=hidden_dim, out_channels=hidden_dim // 4, kernel_size=5, stride=1, padding="same"))
        layers.append(self.activation)
    ...
    def forward(self, error, mask):
        ...
        error = self.output_module(error, mask)
        return error
