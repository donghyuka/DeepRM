...
class CNNModel(torch.nn.Module):
    def __init__(self, input_height, input_width, hidden_dim, output_dim, num_err_layers, num_meta_layers,
        ...
        ## Common Error Module
        layers = []
        layers.append(torch.nn.Conv2d(in_channels=hidden_dim*2, out_channels=hidden_dim, kernel_size=(1,1), stride=1, padding="same"))
        for i in range(num_err_layers):
            layers.append(ResidualBlock2D(kernel_size=kernel_size, in_channels=hidden_dim, out_channels=hidden_dim,
                                          stride=1, padding="same", activation="silu", dropout=dropout_rate))
        ...
        layers.append(torch.nn.Linear(output_dim, output_dim))
    ...
    def forward(self, error, re_error, metadata, pred, mask):
        ...
        x = torch.cat([cm_error, metadata, pred], dim=1)
