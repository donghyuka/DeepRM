...
def extract_move(bam_path, ncpu, bq_cutoff, signal_path_dict, signal_path_arr, intermediate_path):
    ...
    with pysam.AlignmentFile(bam_path, "rb", check_sq=False, threads=ncpu) as input_bam:
        with tqdm.tqdm(total=input_bam.mapped + input_bam.unmapped, desc="Parsing BAM File") as pbar:
            for read_index, read in enumerate(input_bam):
                ...
                data_dict[signal_path].append((read, read_index))
    ...
    for signal_path, data in tqdm.tqdm(data_dict.items(), total=len(data_dict), desc="Saving Move Data"):
        ...
        df_dict = {"mv": [], "read_id": [], "ts": [], "ns": [], "sp": [], "seq": [], "bq": [], "pt": [],
                   "ref": [], "start": [], "cigar": [], "read_index": []}

        for read, read_index in data:
            ...
            df_dict["read_index"].append(read_index)

        move_df = pd.DataFrame.from_dict(df_dict, orient="columns")
        df_len = len(move_df)
        if df_len > 0:
            move_df.to_pickle(f"{intermediate_path}/move_df_postprocess/{signal_path}")
        del move_df
...
def segment_normalize_signal(seg_df_path, signal_path_arr, norm_factor, label_df, pid, norm_mode, token_output_path,
    ...
    for signal_path in tqdm.tqdm(signal_path_arr):
        ...
        if os.path.exists(out_path):
            ...
            continue
        move_path = f"{seg_df_path}/intermediates/move_df_postprocess/{signal_path.split('/')[-1]}"
        ...
        signal_df = signal_df[["bq", "seq", "signal", "dwell_token", "pos", "read_index"]].copy()
        gc.collect()
        ...
        signal_df = signal_df[["segment_len_arr", "signal", "motif", "dwell_motor_token", "dwell_pore_token", "bq", "label_id", "read_index"]].copy()
...
def save_npz(save_path, df):
    ...
    read_index = df["read_index"].values
...
def parse_args():
    ...
    parser.add_argument("--postfix", "-x", type=str, default="postprocess", help="Postfix for output files")
...
def main():
    ...
    os.makedirs(f"{intermediate_path}/move_df_postprocess", exist_ok=True)
    ...
    if os.path.exists(f"{intermediate_path}/move_df_postprocess") and len(glob.glob(f"{intermediate_path}/move_df_postprocess/*.pkl")) > 0:
