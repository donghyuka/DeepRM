...
def parse_bam(bam_path, threads, bq_cutoff):
    ...
    with pysam.AlignmentFile(bam_path, "rb", check_sq=False, threads=threads) as input_bam:
        fetched = input_bam.fetch("NM_000057.4")
        for read_index, read in tqdm.tqdm(enumerate(fetched), desc="Parsing BAM"):
            if read.is_unmapped:
                unmapped += 1
                continue
...
def main():
    ...
    final_df.to_pickle(f"{args.output}/metadata_debug.pkl")
