...
def get_refflat():
    ...
    mane_path = "/extdata4/baeklab/Hyeonseo/m6A/res/ref/mane_table.tsv"
    mane_list = pd.read_csv(mane_path, sep="\t")["RefSeq match transcript (MANE Select)"]
    mane_list = mane_list.dropna()
    mane_list = mane_list.str.split(".", expand=True)[0]
    refflat_df = refflat_df[refflat_df["transcript_id"].isin(mane_list)]
    return refflat_df
...
def main():
    args = parse_args()
    cell1_df = pd.read_pickle(args.cell1)
    cell2_df = pd.read_pickle(args.cell2)
    cell1_df = cell1_df[["genome_id", "pred_dom", "depth"]]
    cell2_df = cell2_df[["genome_id", "pred_dom", "depth"]]
