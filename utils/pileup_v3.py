...
def worker(pid, file_paths, out_path, file_per_worker, threshold_neg = 0.10, threshold_pos = 0.98,
    ...
    keys = ["label_id", "p_sum",
            "logsum_p_pos", "logsum_1_p_pos", "count_pos",
            "logsum_p_neg", "logsum_1_p_neg", "count_neg",
            "logsum_p_all", "logsum_1_p_all", "count_all",
            "dom", "count_dom"]

    key_dict = {"str": ["label_id"],
                "float32": ["p_sum", "logsum_p_pos", "logsum_1_p_pos", "logsum_p_neg", "logsum_1_p_neg",
                            "logsum_p_all", "logsum_1_p_all", "dom"],
                "int32": ["count_pos", "count_neg", "count_all", "count_dom"]}
    ...
    for idx, path in enumerate(tqdm.tqdm(file_paths)):
        ...
        data_df_all["count"] = 1
        data_df_all["logsum_p"] = np.log10(np.clip(data_df_all["pred"].to_numpy(),  epsilon, 1.0))
        data_df_all["logsum_1_p"] = np.log10(1 - np.clip(data_df_all["pred"].to_numpy(), 0.0, 1 - epsilon))
        ...
        data_df_neg = data_df_all[data_df_all["pred"] < threshold_neg]
        data_df_dom = data_df_all[(data_df_all["pred"] < threshold_dom_neg) | (data_df_all["pred"] >= threshold_dom_pos)]

        ## groupby label_id
        data_df_pos = data_df_pos.groupby("label_id").agg({"logsum_p": "sum", "logsum_1_p": "sum", "count": "sum"}).reset_index()
        data_df_neg = data_df_neg.groupby("label_id").agg({"logsum_p": "sum", "logsum_1_p": "sum", "count": "sum"}).reset_index()
        data_df_dom = data_df_dom.groupby("label_id").agg({"dom": "sum", "count": "sum"}).reset_index()
        data_df_all = data_df_all.groupby("label_id").agg({"logsum_p": "sum", "logsum_1_p": "sum", "count": "sum", "pred": "sum"}).reset_index()


        data_df_pos.rename(columns = {"count": "count_pos", "logsum_p": "logsum_p_pos", "logsum_1_p": "logsum_1_p_pos"}, inplace = True)
        data_df_neg.rename(columns = {"count": "count_neg", "logsum_p": "logsum_p_neg", "logsum_1_p": "logsum_1_p_neg"}, inplace = True)
        data_df_all.rename(columns = {"count": "count_all", "pred":"p_sum", "logsum_p": "logsum_p_all", "logsum_1_p": "logsum_1_p_all"}, inplace = True)
        data_df_dom.rename(columns = {"count": "count_dom"}, inplace = True)

        data_df_all = data_df_all.merge(data_df_pos, on = "label_id", how = "left")
        data_df_all = data_df_all.merge(data_df_neg, on = "label_id", how = "left")
        ...
        del data_df_pos, data_df_neg, data_df_dom, data_df_all
        gc.collect()
