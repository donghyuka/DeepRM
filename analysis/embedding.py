...
import tqdm
import gc
import analysis.transformer_embedding as tem
...
def inference_worker(rank, args_dict, flush_interval = 10):
    ...
    kmer_list = []
    kmer_embedding_list = []
    signal_embedding_list = []
    move_list = []
    bq_list = []
    bq_embedding_list = []
    out_dir = f"{args_dict['output']}/embedding/{args_dict['model'].split('/')[-1].split('.')[0]}-{args_dict['data'].split('/')[-1]}/"
    ...
    for idx, data in enumerate(tqdm.tqdm(data_loader, total=len(data_loader), smoothing = 0)):
        data = data[0]
        kmer_list.append(data["kmer_token"])
        ...
        kmer_embedding, signal_embedding, bq_embedding = model(src_kmer, src_signal, src_bq)

        if args_dict["gpu"] > 0:
            kmer_embedding_list.append(kmer_embedding.cpu().detach().numpy())
            signal_embedding_list.append(signal_embedding.cpu().detach().numpy())
            bq_embedding_list.append(bq_embedding.cpu().detach().numpy())

        else:
            kmer_embedding_list.append(kmer_embedding.detach().numpy())
            signal_embedding_list.append(signal_embedding.detach().numpy())
            bq_embedding_list.append(bq_embedding.detach().numpy())

        id_list.append(np.array(data["label_id"]))
        label_list.append(np.array(data["label"]))
        move_list.append(np.array(data["move_token"]))
        bq_list.append(np.array(data["bq_token"]))
        ...
        if idx % flush_interval == 0 and idx > 0:
            ...
            bq_list = np.concatenate(bq_list)
            kmer_embedding_list = np.concatenate(kmer_embedding_list)
            signal_embedding_list = np.concatenate(signal_embedding_list)
            bq_embedding_list = np.concatenate(bq_embedding_list)
            kmer_list = [x for x in kmer_list]
            move_list = [x for x in move_list]
            kmer_embedding_list = [x for x in kmer_embedding_list]
            signal_embedding_list = [x for x in signal_embedding_list]
            bq_list = [x for x in bq_list]
            bq_embedding_list = [x for x in bq_embedding_list]

            data_dict = ({"label_id": id_list, "label": label_list, "kmer": kmer_list, "move": move_list,
                      "kmer_embedding": kmer_embedding_list, "signal_embedding": signal_embedding_list,
                          "bq": bq_list, "bq_embedding": bq_embedding_list})
