...
wdir = "/extdata2/baeklab/Hyeonseo/res/spacer_design_v2"
    ...
    for part_comb in tqdm(part_comb_split):
        ...
        if np.max(composition) - np.min(composition) <= 1:
            ...
            if np.all(part_to_add >= 0):
                ...
                ## if any of element in block_sequence_list exceeds 1000, sample 1000

                print([len(x) for x in block_sequence_list])
                block_sequence_list = [x if len(x) < max_size else sample_from_set(x, max_size) for x in block_sequence_list]
                print([len(x) for x in block_sequence_list])
...
def main(spacer_size = 12, n_blocks=2, cb_pad = 10, ncpu = 120):
    block_list = [f"block_{i}" for i in range(n_blocks)]
    ...
    #
    # sequence_df = pd.read_pickle(f"{wdir}/spacer_candidates_{spacer_size}x{n_blocks}.leven_filtered.pkl")
    #
    ...
    # with open(f"{wdir}/error_dict_{spacer_size}x{n_blocks}.pkl","wb") as f:
    ...
    ## sort by mean_error_rate and select top 30%
    sequence_df = sequence_df[sequence_df["mean_error_rate"] <= 0.02].reset_index(drop=True)
