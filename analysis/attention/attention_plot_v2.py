...
path = "/extdata4/baeklab/Hyeonseo/m6A/inference/attention/BERMUDA-Proto-v48-20240417-184430-0-0-baeklab_v2_depth20_sampled_balanced_drach"
...
def plot_attention(df, comment, minmax_dict):
    ...
    for idx, row in tqdm.tqdm(df.iterrows(), total=len(df)):
        ...
        title_list = []
        title_list += [f"Signal Attention Layer {n}" for n in range(2)]
        title_list += [f"k-mer Attention Layer {n}" for n in range(2)]
        title_list += [f"Merged Attention Layer {n}" for n in range(4)]
        for i in range(8):
            ax = axes[i // 3, i % 3]
        ...
        plt.savefig(f"{path}/plot/{comment}/{row['label_id']}:{idx}.png")
        plt.close()
