...
def inference_worker(rank, args_dict):
    ...
    activation_tensor_list = []

    TransformerModel = importlib.import_module(f"model.{model_config['model']}_debug").TransformerModel
    ...
    def hook_fn(module, input, output):
        activation_tensor_list.append(torch.stack(module.activation_tensor,dim=1).numpy())
        return None
    ...
    for idx, data in tqdm.tqdm(enumerate(data_loader), total=len(data_loader), smoothing = 0):
        ...
        if idx % args_dict["flush"] == 0 and (idx > 0 or args_dict["flush"] == 1):
            id_list = np.concatenate(id_list)
            pred_list = np.concatenate(pred_list)
            activation_tensor_list = np.concatenate(activation_tensor_list, axis=0)

            data_dict = {"label_id": id_list, "pred": pred_list, "activation": activation_tensor_list}
