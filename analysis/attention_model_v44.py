...
class TransformerModel(nn.Module):
    ...
    def __init__(self, d_model: int, n_heads: int, d_ff: int,
        ...
        self.kmer_embedding = nn.Embedding(self.kmer_count, d_model//16)
        ...
        self.regression_head = RegressionHead(d_model+d_model//16,d_model, lin_act, lin_depth, lin_dropout, seq_len)
    ...
    def forward(self, src_kmer: Tensor, src_signal: Tensor,
        ...
        signal_output, attn_list = self.transformer_encoder(src=signal_output, mask = None, src_key_padding_mask = src_pad_mask)
        signal_output = torch.cat([signal_output, kmer_embedding], dim = -1)

        ## apply regression head to each token:
        output = self.regression_head(signal_output)
...
class RegressionHead(nn.Module):
    def __init__(self, d_model_in: int, d_model: int, lin_act: str, lin_depth: int, lin_dropout: float, seq_length: int):
        super().__init__()
        layer_list=  []

        layer_list.append(nn.Linear(d_model_in, d_model))
