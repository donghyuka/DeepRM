...
def parse_args():
    ...
    parser.add_argument("--min_depth", "-m", type=int, default=1, help="Minimum depth")
...
def filter_pileup(args):
    """
    Filters the pileup file based on the provided arguments.
...
def filter_pileup_proc(args, df_proc, collect_dict, pid):
    """
    Worker function to filter a subset of the pileup DataFrame.

    Args:
        args (argparse.Namespace): Parsed command-line arguments.
        df_proc (pd.DataFrame): Subset of the pileup DataFrame to process.
        collect_dict (multiprocessing.Manager.dict): Dictionary to collect results from worker processes.
        pid (int): Process ID.
...
def process_df(df, args):
    """
    Processes a DataFrame by creating 5-mer motifs and filtering based on depth and base.
...
def main():
    """
    Main function to parse arguments and filter the pileup file.
