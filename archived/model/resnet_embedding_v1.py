...
class ResNetModel(nn.Module):
    ...
    def __init__(self, n_layers_kmer: int, n_layers_sig: int, n_layers_merged: int, n_layers_final: int,
                 d_kmer: int, d_signal: int, d_merged: int, d_final: int, d_embedding: int = 512,
        ...
        self.start_module_kmer = [nn.AvgPool1d(5,5,0),
                                  nn.Conv1d(4*kmer_size, d_kmer, kernel_size, stride = 1, padding = 'same'),
        ...
        self.start_module_embedding = [nn.Conv1d(d_embedding, d_signal, kernel_size, stride = 1, padding = 'same'),
                                       nn.BatchNorm1d(d_signal), act_func]

        self.start_module_signal_embedding = [nn.Conv1d(d_signal*2, d_signal*2, 1, stride = 1, padding = 'same'),
                                              nn.BatchNorm1d(d_signal*2), act_func]

        self.residual_module_signal_2 = []
        for _ in range(n_layers_sig):
            self.residual_module_signal_2.append(ResidualBlock(kernel_size, d_signal*2, d_signal*2, stride = 1,
                                                             padding = 'same', activation = act, dropout = dropout))


        self.residual_module_merged = [ResidualBlock(kernel_size, d_signal*2+d_kmer, d_merged, stride = 1,
                                                       padding = 'same', activation = act, dropout = dropout),]
        ...
        self.start_module_embedding = nn.Sequential(*self.start_module_embedding)
        self.start_module_signal_embedding = nn.Sequential(*self.start_module_signal_embedding)
        ...
        self.start_module_embedding = nn.SyncBatchNorm.convert_sync_batchnorm(self.start_module_embedding)
        self.start_module_signal_embedding = nn.SyncBatchNorm.convert_sync_batchnorm(self.start_module_signal_embedding)
    ...
    def forward(self, src_embedding: Tensor, src_kmer: Tensor, src_signal: Tensor, src_bq: Tensor, src_move: Tensor,
                src_pad_mask: Tensor) -> Tensor:

        pad_mask = (src_pad_mask == 0).float().unsqueeze(1)
        ...
        embedding_out = self.start_module_embedding(src_embedding) * pad_mask
        signal_out = torch.cat([signal_out, embedding_out], dim = 1)
        signal_out = self.start_module_signal_embedding(signal_out) * pad_mask
