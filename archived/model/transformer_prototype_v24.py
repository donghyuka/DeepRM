...
class TransformerModel(nn.Module):
    ...
    def forward(self, src_kmer: Tensor, src_signal: Tensor, src_spectrogram: Tensor, src_bq: Tensor, src_move: Tensor,
        ...
        src_bq = src_bq.unsqueeze(-1)
        div_term = torch.exp(torch.arange(0, self.d_model, 2) * (-math.log(2*self.max_bq) / self.d_model)).to(src_bq.device)
        bq_embedding = torch.zeros(src_bq.shape[0], src_bq.shape[1], self.d_model).to(src_bq.device)
        bq_embedding[:, :, 0::2] = torch.sin(src_bq * div_term)
        bq_embedding[:, :, 1::2] = torch.cos(src_bq * div_term)

        src_move = src_move.unsqueeze(-1)
        div_term = torch.exp(torch.arange(0, self.d_model, 2) * (-math.log(2*self.block_len) / self.d_model)).to(src_bq.device)
        mv_embedding = torch.zeros(src_move.shape[0], src_move.shape[1], self.d_model).to(src_move.device)
        ...
        final_embedding = torch.stack([kmer_embedding, signal_embedding, bq_embedding, pos_encoding, mv_embedding], dim = 0).sum(dim = 0)
...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, seq_len: int) -> None:
        ...
        div_term = torch.exp(torch.arange(0, d_model, 2) * (-math.log(2*seq_len) / d_model))
