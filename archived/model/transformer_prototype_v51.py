...
class TransformerModel(nn.Module):
    ...
    def __init__(self, d_model: int, n_heads: int, d_ff: int,
        ...
        self.kmer_embedding = nn.Embedding(self.kmer_count+1, d_model)
        ...
        encoder_layer = nn.TransformerEncoderLayer(d_model, n_heads, d_ff, dropout = encoder_dropout, activation = t_act,
                                                   batch_first=True)
        self.transformer_encoder_1 = nn.TransformerEncoder(encoder_layer, 3)
        self.transformer_encoder_2 = nn.TransformerEncoder(encoder_layer, 3)
        self.transformer_encoder_3 = nn.TransformerEncoder(encoder_layer, 4)
    ...
    def forward(self, src_kmer: Tensor, src_signal: Tensor, src_bq: Tensor, src_move: Tensor, src_pad_mask: Tensor, target_mask: Tensor) -> Tensor:
        ...
        kmer_output = torch.add(kmer_embedding, bq_embedding)
        kmer_output = torch.add(kmer_output, pos_encoding)
        ...
        pad_mask_inverse = (src_pad_mask == 0)
        pad_mask_sum = pad_mask_inverse.sum(dim = 1)
        output = output * pad_mask_inverse
