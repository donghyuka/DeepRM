...
class TransformerModel(nn.Module):
    ...
    def __init__(self, d_model: int, n_heads: int, d_ff: int,
        ...
        self.bq_embedding = nn.Linear(max_bq, d_model)
    ...
    def forward(self, src_kmer: Tensor, src_signal: Tensor, src_spectrogram: Tensor, src_bq: Tensor, src_move: Tensor,
        ...
        src_bq_list = []
        for bq_cutoff in range(1, self.max_bq+1):
            ## clamp at the maximum bq
            bq_tensor = torch.nn.functional.one_hot(torch.clamp(src_bq, 0, bq_cutoff), num_classes=bq_cutoff+1)
            bq_tensor = F.pad(bq_tensor, (0, self.max_bq - bq_cutoff, 0, 0), value=0)
            src_bq_list.append(bq_tensor)
        src_bq = torch.sum(torch.stack(src_bq_list, dim = 0), dim = 0).float()
        src_bq = src_bq[:,:,1:]
