...
class TransformerModel(nn.Module):
    ...
    def __init__(self, d_model: int, n_heads: int, d_ff: int,
        ...
        encoder_layer = nn.TransformerEncoderLayer(d_model, n_heads, d_ff, dropout = encoder_dropout, activation = t_act,
                                                   batch_first=True)
        self.transformer_encoder_1 = nn.TransformerEncoder(encoder_layer, 2)
        self.transformer_encoder_2 = nn.TransformerEncoder(encoder_layer, 2)
        self.transformer_encoder_3 = nn.TransformerEncoder(encoder_layer, n_layers)
    ...
    def forward(self, src_kmer: Tensor, src_signal: Tensor, src_bq: Tensor, src_move: Tensor, src_pad_mask: Tensor, target_mask: Tensor) -> Tensor:
        ...
        signal_output = self.transformer_encoder_1(src=signal_output, mask = None, src_key_padding_mask = src_pad_mask)
        kmer_output = torch.add(kmer_embedding, pos_encoding)
        kmer_output = self.transformer_encoder_2(src=kmer_output, mask = None, src_key_padding_mask = src_pad_mask)
        cross_output = self.cross_attention_layer(kmer_output, signal_output, signal_output, key_padding_mask = src_pad_mask,
                                                  need_weights = False, is_causal = False)
        cross_output = cross_output[0] + signal_output
        cross_output = self.layer_norm(cross_output)
        cross_output = self.transformer_encoder_3(src=cross_output, mask = None, src_key_padding_mask = src_pad_mask)
