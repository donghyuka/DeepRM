...
class TransformerModel(nn.Module):
    ...
    def __init__(self, d_model: int, n_heads: int, d_ff: int,
        ...
        self.kmer_embedding = nn.Embedding(4**kmer_size+1, d_model//2)
        self.bq_embedding = nn.Embedding(max_bq+1, d_model//2)
    ...
    def forward(self, src_kmer: Tensor, src_signal: Tensor, src_spectrogram: Tensor, src_bq: Tensor, src_move: Tensor,
        ...
        pos_encoding = self.pos_encoding(torch.zeros_like(move_embedding))

        ## add all embeddings and dropout
        final_embedding = torch.concat([kmer_embedding, bq_embedding], dim = -1)
        final_embedding = torch.stack([final_embedding, pos_encoding, move_embedding], dim = 0).sum(dim = 0)
