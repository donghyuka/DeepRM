...
class TransformerModel(nn.Module):
    ...
    def forward(self, src_kmer: Tensor, src_signal: Tensor, src_bq: Tensor, src_move: Tensor,
        ...
        output = torch.stack([signal_embedding, pos_encoding], dim = 0).sum(dim = 0)
        output = self.transformer_encoder_1(src=output, mask = None, src_key_padding_mask = src_pad_mask)
        output = torch.add(output, kmer_embedding)
        output = self.transformer_encoder_2(src=output, mask = None, src_key_padding_mask = src_pad_mask)
