...
class ResNetModel(nn.Module):
    ...
    def __init__(self, n_layers_kmer: int, n_layers_sig: int, n_layers_merged: int, n_layers_final: int,
        ...
        self.residual_module_kmer_1 = []
        for _ in range(n_layers_kmer):
            self.residual_module_kmer_1.append(ResidualBlock(kernel_size, d_kmer, d_kmer, stride = 1, padding = 'same',
                                                           activation = act, dropout = dropout))
        self.residual_module_kmer_2 = []
        for _ in range(n_layers_kmer):
            self.residual_module_kmer_2.append(ResidualBlock(kernel_size, d_kmer, d_kmer, stride = 1, padding = 'same',
                                                           activation = act, dropout = dropout))
        self.residual_module_kmer_3 = []
        for _ in range(n_layers_kmer):
            self.residual_module_kmer_3.append(ResidualBlock(kernel_size, d_kmer, d_kmer, stride = 1, padding = 'same',
                                                           activation = act, dropout = dropout))
        self.residual_module_kmer_4 = []
        for _ in range(n_layers_kmer):
            self.residual_module_kmer_4.append(ResidualBlock(kernel_size, d_kmer, d_kmer, stride = 1, padding = 'same',
                                                           activation = act, dropout = dropout))
        ...
        for _ in range(n_layers_sig):
            self.residual_module_signal_2.append(ResidualBlock(kernel_size, d_signal, d_signal, stride = 1,
                                                             padding = 'same', activation = act, dropout = dropout))
        self.residual_module_signal_3 = []
        for _ in range(n_layers_sig):
            self.residual_module_signal_3.append(ResidualBlock(kernel_size, d_signal, d_signal, stride = 1,
                                                             padding = 'same', activation = act, dropout = dropout))
        self.residual_module_signal_4 = []
        for _ in range(n_layers_sig):
            self.residual_module_signal_4.append(ResidualBlock(kernel_size, d_signal, d_signal, stride = 1,
                                                             padding = 'same', activation = act, dropout = dropout))
        ...
        self.residual_module_kmer_1 = nn.Sequential(*self.residual_module_kmer_1)
        self.residual_module_kmer_2 = nn.Sequential(*self.residual_module_kmer_2)
        self.residual_module_kmer_3 = nn.Sequential(*self.residual_module_kmer_3)
        self.residual_module_kmer_4 = nn.Sequential(*self.residual_module_kmer_4)
        self.residual_module_signal_1 = nn.Sequential(*self.residual_module_signal_1)
        self.residual_module_signal_2 = nn.Sequential(*self.residual_module_signal_2)
        self.residual_module_signal_3 = nn.Sequential(*self.residual_module_signal_3)
        self.residual_module_signal_4 = nn.Sequential(*self.residual_module_signal_4)
    ...
    def forward(self, src_kmer: Tensor, src_signal: Tensor, src_bq: Tensor, src_move: Tensor, target_mask: Tensor) -> Tensor:
        ...
        kmer_out = self.residual_module_kmer_1(kmer_out)
        signal_out = self.start_module_signal(src_signal) * pad_mask
        signal_out = self.residual_module_signal_1(signal_out)

        pad_mask = torch.nn.MaxPool1d(2,2,0)(pad_mask)

        kmer_out = torch.nn.AvgPool1d(2,2,0)(kmer_out) * pad_mask
        kmer_out = self.residual_module_kmer_2(kmer_out)
        signal_out = torch.nn.AvgPool1d(2,2,0)(signal_out) * pad_mask
        signal_out = self.residual_module_signal_2(signal_out)

        pad_mask = torch.nn.MaxPool1d(2,2,0)(pad_mask)

        kmer_out = torch.nn.AvgPool1d(2,2,0)(kmer_out)* pad_mask
        kmer_out = self.residual_module_kmer_3(kmer_out)
        signal_out = torch.nn.AvgPool1d(2,2,0)(signal_out)* pad_mask
        signal_out = self.residual_module_signal_3(signal_out)

        pad_mask = torch.nn.MaxPool1d(2,2,0)(pad_mask)

        kmer_out = torch.nn.AvgPool1d(2,2,0)(kmer_out)* pad_mask
        kmer_out = self.residual_module_kmer_4(kmer_out)
        signal_out = torch.nn.AvgPool1d(2,2,0)(signal_out)* pad_mask
        signal_out = self.residual_module_signal_4(signal_out)

        output = torch.cat([kmer_out, signal_out], dim = 1) * pad_mask
        output = self.residual_module_merged(output) * pad_mask
        output = output.permute(0, 2, 1)
        output = self.regression_head(output).squeeze(-1)
