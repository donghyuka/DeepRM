...
class TransformerModel(nn.Module):
    ...
    def __init__(self, d_model: int, n_heads: int, d_ff: int,
        ...
        self.transformer_encoder_2 = nn.TransformerEncoder(encoder_layer, 1)
        self.transformer_encoder_3 = nn.TransformerEncoder(encoder_layer, 3)
    ...
    def forward(self, src_kmer: Tensor, src_signal: Tensor, src_bq: Tensor, src_move: Tensor, src_pad_mask: Tensor, target_mask: Tensor) -> Tensor:
        ...
        signal_output = torch.add(signal_embedding, move_embedding)
        signal_output = torch.add(signal_output, bq_embedding)
        signal_output = self.transformer_encoder_1(src=signal_output, mask = None, src_key_padding_mask = src_pad_mask)
        kmer_output = torch.add(kmer_embedding, move_embedding)
