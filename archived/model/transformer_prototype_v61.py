...
class TransformerModel(nn.Module):
    ...
    def __init__(self, d_model: int, n_heads: int, d_ff: int,
        ...
        encoder_layer = nn.TransformerEncoderLayer(d_model, n_heads, d_model * 4, dropout = encoder_dropout, activation = t_act,
                                                   batch_first=True)
        self.transformer_encoder = nn.TransformerEncoder(encoder_layer, n_layers)

        ## Regression Head Initialization
        self.regression_head_1 = RegressionHead(d_ff, d_model, 32, lin_act, lin_depth, lin_dropout, seq_len)
        self.regression_head_2 = RegressionHead(d_ff, 6400, 1, lin_act, lin_depth, 0, seq_len)
...
class RegressionHead(nn.Module):
    def __init__(self, d_model: int,  in_dim: int, out_dim: int, lin_act: str, lin_depth: int, lin_dropout: float, seq_length: int):
        ...
        for i in range(lin_depth-4):
            layer_list.append(nn.Linear(d_model, d_model))
            layer_list.append(self._get_activation_fn(lin_act))
            layer_list.append(nn.Dropout(lin_dropout))

        layer_list.append(nn.Linear(d_model, d_model//2))
        layer_list.append(self._get_activation_fn(lin_act))
        layer_list.append(nn.Linear(d_model//2, d_model//4))
        layer_list.append(self._get_activation_fn(lin_act))
        layer_list.append(nn.Linear(d_model//4, d_model//8))
        layer_list.append(self._get_activation_fn(lin_act))
        layer_list.append(nn.Linear(d_model//8, out_dim))
