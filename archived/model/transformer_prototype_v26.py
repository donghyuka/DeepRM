...
class TransformerModel(nn.Module):
    ...
    def forward(self, src_kmer: Tensor, src_signal: Tensor, src_spectrogram: Tensor, src_bq: Tensor, src_move: Tensor,
        ...
        div_term = torch.exp(torch.arange(0, self.d_model // 4, 2) * (-math.log(10000.0) / self.d_model // 4)).to(src_bq.device)
        bq_embedding = torch.zeros(src_bq.shape[0], src_bq.shape[1], self.d_model // 4).to(src_bq.device)
        bq_embedding[:, :, 0::2] = torch.sin(src_bq * div_term)
        bq_embedding[:, :, 1::2] = torch.cos(src_bq * div_term)

        src_move = src_move.unsqueeze(-1)
        mv_embedding = torch.zeros(src_move.shape[0], src_move.shape[1], self.d_model // 4).to(src_move.device)
        ...
        pos_encoding = torch.concat([pos_encoding, bq_embedding, mv_embedding], dim = -1)
