...
class NanoporeDatasetIterator:
    ...
    def _read_df(self):
        ...
        df["mfe"] = df["motif"].apply(lambda x: RNA.fold(x)[1])
        df["kmer_token"] = df["motif"].apply(lambda x: np.array(list(x)))
        df["bq_token"] = df["bq"].apply(lambda x: x[self.trim:-self.trim])
        df = df[["kmer_token", "signal_token", "bq_token","mfe","segment_len_arr","label_id","block_id"]][df["signal_token"].notnull()].copy()
