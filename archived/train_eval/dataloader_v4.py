...
class BinaryClassDatasetIterator:
    def __init__(self, pos_file_paths, neg_file_paths, buffer_size, read_every = 1000, shuffle = True, class_ratio = 0.5,
                 soft_label = False):
        ...
        self.columns = ["kmer_token", "bq_token", "signal_token", "move_token", "target_mask"]
    ...
    def _read_data(self, first_read = False):


        if first_read:
            fill_to = self.buffer_size
            df_list = []

        else:
            fill_to = self.read_every
            df_list = [self.buffer_df[self.current_class]]

        len_read = 0
        while (len_read < fill_to) and (self.current_df_index[self.current_class] < len(self.paths[self.current_class]) - 1):
    ...
    def _give_df_to_iterator(self, df):
        ...
        df["kmer_token"] = df["kmer_token"].apply(lambda x: x.astype(np.int16))
...
class NanoporeDataset(torch.utils.data.IterableDataset):
    ...
    def __iter__(self):
        ...
        return BinaryClassDatasetIterator(pos_file_paths, neg_file_paths, buffer_size = self.buffer_size, read_every=self.read_every,
                                          shuffle = self.shuffle, class_ratio = class_ratio_iterator, soft_label = self.soft_label)
...
class NanoporeDataLoader(DataLoader):
    def __init__(self, dataset:NanoporeDataset, batch_size, num_workers, pin_memory, drop_last, collate_fn, prefetch_factor):
        ...
        super().__init__(dataset, batch_size=batch_size, num_workers=8, pin_memory=pin_memory, persistent_workers=True,
                         drop_last=drop_last, shuffle=shuffle, sampler=sampler, collate_fn=collate_fn,
                         prefetch_factor=8)

    def __len__(self):
        return len(self.dataset) // self.batch_size * self.num_workers
