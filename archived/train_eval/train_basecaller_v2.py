...
from archived.train_eval.dataloader_v2 import load_dataset, NanoporeDataLoader
...
def parse_args():
    ...
    if args.name is None:
        args.name = f"BERMUDA-Basecaller-{args.model.split('_')[-1]}-{strfttime}"
...
class Trainer:
    def __init__(
        ...
        self.loss_func = functools.partial(self._loss_func)
    ...
    def _loss_func(self, output_basecalling, target_basecalling):
        weight = torch.ones(6, dtype=torch.float).to(self.gpu_id)
        ## Channel order: [PAD, cA, C, G, T, m6A]
        weight[0] = 0.0
        weight[-1] = self.loss_ratio
    ...
    def _feed_model(self, source, target_modification):
        ...
        return output_basecalling, output_modification, target_basecalling


    def _run_batch(self, source, target):
        self.optimizer.zero_grad()
        output_basecalling, output_modification, target_basecalling = self._feed_model(source, target)
        loss = self.loss_func(output_basecalling, target_basecalling)
        loss.backward()
        if self.grad_clip > 0:
