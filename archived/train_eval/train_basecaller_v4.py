...
def kmer_to_nuc_tensor(kmer_tensor, m6a, target_mask, src_pad_mask):
    ...
    nuc_tensor = ((kmer_tensor-1)%64)//16
    nuc_tensor = nuc_tensor + (target_mask * m6a.unsqueeze(-1) * 4).long() + 1
    ## Reserve 0 for padding.
    nuc_tensor = nuc_tensor * (src_pad_mask == 0)
    nuc_tensor = 5 - nuc_tensor
    return nuc_tensor
...
class Trainer:
    ...
    def _loss_func(self, output_basecalling, output_modification, target_basecalling, target_modification, loss_ratio=1.0):
        ...
        if loss_ratio <= 10000:
            weight = torch.tensor([20.0,1.0,1.0,1.0,0.83333]).to(self.gpu_id)
            ## Channel order: [m6A, U, G, C, cA, PAD]
            basecalling_loss = torch.nn.CrossEntropyLoss(reduction="mean", ignore_index = 5, weight=weight)(output_basecalling, target_basecalling)
