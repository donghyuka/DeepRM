...
from archived.train_eval.dataloader_od import load_dataset, NanoporeDataLoader
...
class Trainer:
    ...
    def _forward_model(self, source, target_scalar, return_scalar = False):
        ...
        target_scalar = target_scalar.to(torch.float32).to(self.gpu_id)
        target_vector = target_scalar.unsqueeze(1)
        target_vector = target_vector * src_target_mask

        output_vector = self.model(src_kmer, src_signal, src_bq, src_pad_mask)

        if return_scalar:
            target_mask_sum = src_target_mask.sum(dim = 1)
            output_scalar = output_vector * src_target_mask
