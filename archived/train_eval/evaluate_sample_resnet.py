...
from archived.train_eval.inference_dataloader_resnet import load_dataset
...
def inference_worker(rank, args_dict):
    ...
    ResNetModel = importlib.import_module(f"model.{model_config['model']}").ResNetModel

    model = ResNetModel(n_layers_kmer = model_config["n_layers_kmer"], n_layers_sig = model_config["n_layers_sig"],
                        n_layers_merged = model_config["n_layers_merged"], n_layers_final = model_config["n_layers_final"],
                        d_kmer = model_config["d_kmer"], d_signal = model_config["d_signal"], d_merged = model_config["d_merged"],
                        d_final = model_config["d_final"], dropout = model_config["dropout"], kernel_size = model_config["kernel_size"])
    if rank == 0:
        total_params = 0
    ...
    for data in tqdm.tqdm(data_loader, total=len(data_loader), smoothing = 0):
        ...
        with torch.no_grad():
            if score_feature:
                pred = model(src_kmer, src_signal, src_bq, src_move, src_target_mask, src_score)
            else:
                pred = model(src_kmer, src_signal, src_bq, src_move, src_target_mask)
