...
from archived.segment_normalize_signal import segment_signal
...
def extract_move(bam_path,ncpu,bq_cutoff, signal_path_dict, signal_path_arr, intermediate_path):
    ...
    for signal_path, data in tqdm.tqdm(data_dict.items(), total=len(data_dict), desc="Saving Move Data"):
        ...
        df_len = len(move_df)
        if df_len > 0:
            move_df.to_pickle(f"{intermediate_path}/move_df_split_delta_v2/{signal_path.split('/')[-1]}")
        del move_df
...
def segment_normalize_fft_signal(seg_df_path, pid, wdir_path, signal_path_arr, label_df, sampling_ratio = None,
    ...
    for signal_path in tqdm.tqdm(signal_path_arr):
        ...
        if os.path.exists(out_path):
            ...
            continue
        move_path = f"{wdir_path}/move_df_split_delta_v2/{signal_path.split('/')[-1]}"
        if not os.path.exists(move_path):
            print(f"Move data not found: {move_path}")
            continue
        ...
        if len(signal_df) == 0:
            print(f"Empty signal data (POS): {signal_path}")
            continue
        ...
        if len(signal_df) == 0:
            print(f"Empty signal data (TS): {signal_path}")
            continue
        ...
        signal_df = signal_df[["read_id", "ref", "bq", "seq", "signal_seg", "pos"]].copy()
        gc.collect()
        signal_df = signal_df[signal_df["pos"].apply(lambda x: len(x) > 0)]
        ...
        signal_df = signal_df[['label_id', 'motif', 'signal_seg', 'bq', 'label']].copy()
        gc.collect()
        #
        ...
        #
        # signal_df["signal_token"] = signal_df.apply(lambda x: segmented_signal_to_block(x["signal_seg"], x["segment_len_arr"], kmer, sampling, sig_window), axis=1)
        #
        # signal_df["segment_len_arr"] = signal_df["segment_len_arr"].apply(lambda x: x[trim:-trim])
        #
        # signal_df["kmer_token"] = signal_df.apply(lambda x: expand_token_to_segment(sequence_to_kmer_token(x["motif"], kmer), x["segment_len_arr"]), axis=1)
        #
        #
        ...
        #
        # signal_df = signal_df[["block_id", "label_id", "label", "motif", "kmer_token", "bq_token", "signal_token",
        #                        "move_token", "target_mask"]][signal_df["signal_token"].notnull()].copy()
...
def parse_args():
    ...
    parser.add_argument("--pod5", "-p", type=str, default="/extdata4/baeklab/Hyeonseo/m6A/runs/exp_MRNA/ON0090/ON0090/raw/pod5", help="POD5 Input directory")
    parser.add_argument("--bam", "-b", type=str, default="/extdata4/baeklab/Hyeonseo/m6A/runs/exp_MRNA/ON0090/ON0090/result/dorado/dorado_output.sorted.filtered.bam", help="Dorado BAM file")
    parser.add_argument("--qcut", "-q", type=int, default=0, help="BQ cutoff")
    parser.add_argument("--wdir", "-w", type=str, default="/extdata4/baeklab/Hyeonseo/m6A/runs/exp_MRNA/ON0090/ON0090/result/block/intermediates", help="Working directory")
    ...
    parser.add_argument("--label", "-l", type=str, default="/extdata4/baeklab/Hyeonseo/m6A/runs/exp_MRNA/ON0090/ON0090/raw/label.tsv", help="Label file")
...
def main():
    ...
    os.makedirs(f"{intermediate_path}/move_df_split_delta_v2", exist_ok=True)
