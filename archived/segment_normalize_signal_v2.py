...
def extract_move(bam_path, ncpu, signal_path_dict, signal_path_arr, intermediate_path):
    ...
    data_dict = {x: {"mv": [], "read_id": [], "sm": [], "sd": [], "ts": [],
                     "pi": [],

                     } for x in signal_path_arr}
    ...
    for signal_path, data in tqdm.tqdm(data_dict.items(), total=len(data_dict), desc="Saving Move Data"):
        ...
        df_len = len(move_df)
        if df_len > 0:
            move_df.to_pickle(f"{intermediate_path}/move_df_split_v2/{signal_path}")
        del move_df
...
def segment_normalize_fft_signal(seg_df_path, signal_path_arr):
    for signal_path in tqdm.tqdm(signal_path_arr):
        ...
        if not os.path.exists(signal_path):
            continue
        if not os.path.exists(f"{seg_df_path}/intermediates/move_df_split_v2/{signal_path.split('/')[-1]}"):
            continue
        if not os.path.exists(f"{seg_df_path}/intermediates/block_df_split_v2/{signal_path.split('/')[-1]}"):
            continue
        ...
        move_df = pd.read_pickle(f"{seg_df_path}/intermediates/move_df_split_v2/{signal_path.split('/')[-1]}")
        ...
        block_df = pd.read_pickle(f"{seg_df_path}/intermediates/block_df_split_v2/{signal_path.split('/')[-1]}")
...
def split_block_df(args, signal_path_dict, signal_path_arr, intermediate_path):
    ...
    for signal_path, group_df in tqdm.tqdm(block_df_groupby, total = len(signal_path_arr), desc="Splitting Block Dataframe"):
        group_df.to_pickle(f"{intermediate_path}/block_df_split_v2/{signal_path}")
...
def main():
    ...
    signal_raw_path = f"{intermediate_path}/signal_raw_v2/"
    signal_index_path = f"{intermediate_path}/signal_index_v2.pkl"

    os.makedirs(intermediate_path, exist_ok=True)
    os.makedirs(signal_raw_path, exist_ok=True)
    os.makedirs(f"{intermediate_path}/move_df_split_v2", exist_ok=True)
    os.makedirs(f"{intermediate_path}/block_df_split_v2", exist_ok=True)
