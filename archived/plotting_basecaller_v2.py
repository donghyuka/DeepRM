...
def process_inferece(data_df, updated_label_path, outdir, modelname, baseline_pred, dorado_pred, ratio, min_depth = 5):

    plot_histogram_read(data_df, outdir, modelname)
    # threshold = gmm_em_lda(data_df["pred"].values)
    threshold_neg = 0.9 ## pre-calculated threshold using GMM-EM-LDA (It is too slow to calculate every time)
    threshold_pos = 0.8
    threshold_dom_neg = 0.70
    threshold_dom_pos = 0.70

    epsilon = 1e-12
    data_df_bin = data_df[(data_df["pred"] >= threshold_pos) | ( data_df["pred_ca"] >= threshold_neg)].copy()
    data_df_bin = data_df_bin.reset_index(drop = True)

    data_df_dom = data_df[(data_df["pred"] >= threshold_dom_pos) | ( data_df["pred_ca"] >= threshold_dom_neg)].copy()
    ...
    if baseline_pred is not None:
        data_df = data_df.merge(baseline_pred, on = "label_id", how = "outer")
    ...
    data_df = data_df.merge(updated_label_df, on = "label_id", how = "right")
    ## Fill NA with 0
    data_df.fillna(0, inplace = True)
...
def main():
    ...
    for idx, data_path in enumerate(args.input):
        ...
        data_df_original.dropna(subset = ["pred"], inplace = True)
