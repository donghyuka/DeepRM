...
class NanoporeDatasetIterator:
    ...
    def df_to_tensor(self, df, pad_to):
        ...
        source_tensor = torch.cat([source_tensor, torch.ones(len_df, 1)], dim = 1)
        label_tensor = torch.tensor([label, m6a_level, len_df], dtype=torch.float32)


        if len_df < pad_to:
            pad_tensor = torch.zeros(pad_to - len_df, 71)
            source_tensor = torch.cat([source_tensor, pad_tensor], dim = 0)

        elif len_df > pad_to:
            source_tensor = source_tensor[:pad_to]
