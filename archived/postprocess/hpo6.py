...
while True:
    ...
    num_err_layers = np.random.choice([1, 2, 4])
    num_output_layers = np.random.choice([1, 2, 4])
    num_meta_layers = np.random.choice([1, 2, 4])
    num_pred_layers = np.random.choice([1, 2, 4])
    weight_decay = np.random.choice([0.3, 0.1, 0.03])
    dropout = np.random.choice([0.3, 0.5, 0.7])
    lr = np.random.choice([3e-3, 1e-3, 3e-4, 1e-4])
    hidden_dim = np.random.choice([16, 32, 64])
    kernel_size = np.random.choice([3, 5])
    loss = np.random.choice(["Asparagus", "Brassica", "Clover", "Dracena", "Echeveria", "Freesia"])
