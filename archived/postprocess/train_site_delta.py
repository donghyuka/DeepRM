...
from archived.postprocess.dataloader_site_delta import load_dataset, NanoporeDataLoader
...
def parse_args():
    ...
    parser.add_argument("--data", type=str, default="/extdata4/baeklab/Hyeonseo/m6A/postprocess/dataset_v6")
    ...
    parser.add_argument("--model", type=str, default="cnn_model_v2")
    ...
    parser.add_argument("--num_output_layers", type=int, default=8)
    ...
    if args.name is None:
        args.name = f"Postprocess-ResNet-{args.model.split('_')[-1]}-{strfttime}"
...
def main_worker(rank, args_dict):
    ...
    metric_func_dict = {
                        "RMSE": tm.MeanSquaredError(squared=False).to(gpu_id),}
