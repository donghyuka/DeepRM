...
from postprocess.preprocess.inference_dataloader_meta import load_dataset
...
def inference_worker(rank, args_dict, flush_interval = 100):
    ...
    for idx, data in tqdm.tqdm(enumerate(data_loader), total=len(data_loader), smoothing = 0):
        ...
        meta_list.append(np.array(data["metadata"]))
        dom_list.append(np.array(data["dom"]))
        ...
        if idx % flush_interval == 0 and idx > 0:
            ...
            meta_list = np.concatenate(meta_list)
            dom_list = np.concatenate(dom_list)

            data_df = pd.DataFrame({"label_id": id_list, "label": label_list, "pred": pred_list, "block_id": block_id_list,
                                    "metadata": meta_list, "dom": dom_list})
