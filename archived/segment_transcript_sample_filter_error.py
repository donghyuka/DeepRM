...
def extract_move(bam_path,ncpu,bq_cutoff, signal_path_dict, signal_path_arr, intermediate_path):
    ...
    data_dict = {x: {"mv": [], "read_id": [], "sm": [], "sd": [], "ts": [], "seq": [], "bq": [], "md": [],
...
def segment_normalize_fft_signal(seg_df_path, pid, wdir_path, signal_path_arr, label_df, sampling_ratio = None,
    ...
    for signal_path in tqdm.tqdm(signal_path_arr):
        ...
        signal_df = signal_df[signal_df["pos"].notnull()].copy()
...
def ref_pos_to_query_pos(ref_pos_list, cigar, start_pos, query_len, md_tag, cb_pad = 10):
    ...
    query_pos_list = list(query_pos_dict.values())
    ...
    for query_pos in query_pos_list:
        ...
        if query_pos is None:
            ...
            error_slice = np.sum(error_slice)
...
def get_label_pos_list(ref, start, cigar, query_len, md_tag, label_df):
    ...
    query_pos_list = ref_pos_to_query_pos(ref_pos_list, cigar, start, query_len, md_tag)
