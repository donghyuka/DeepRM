...
def segment_normalize_fft_signal(seg_df_path, pid, wdir_path, signal_path_arr, label_df, sampling_ratio = None,
    ...
    for signal_path in tqdm.tqdm(signal_path_arr):
        ...
        signal_df["ref_query_error_label_genomeid"] = pos_list
        signal_df = signal_df[signal_df["ref_query_error_label_genomeid"].notnull()].copy()
        signal_df = signal_df[signal_df["ref_query_error_label_genomeid"].apply(lambda x: len(x) > 0)]
        if len(signal_df) == 0:
            continue
        ...
        signal_df = signal_df[["read_id", "ref", "bq", "seq", "signal_seg", "ref_query_error_label_genomeid", "mapq", "flag", "pi"]].copy()
        signal_df["read_bq"] = signal_df["bq"].apply(mean_phred)
        gc.collect()
        signal_df = signal_df[signal_df["ref_query_error_label_genomeid"].apply(lambda x: len(x) > 0)]
        signal_df = signal_df.explode("ref_query_error_label_genomeid").reset_index(drop=True)

        if len(signal_df) == 0:
            continue

        signal_df["ref"] = signal_df["ref"].str.split(".").str[0]
        signal_df["ref_pos"] = signal_df["ref_query_error_label_genomeid"].apply(lambda x: x[0])
        signal_df["query_pos"] = signal_df["ref_query_error_label_genomeid"].apply(lambda x: x[1])
        signal_df["error"] = signal_df["ref_query_error_label_genomeid"].apply(lambda x: x[2])
        signal_df["label"] = signal_df["ref_query_error_label_genomeid"].apply(lambda x: x[3])
        signal_df["genome_id"] = signal_df["ref_query_error_label_genomeid"].apply(lambda x: x[4])
        signal_df.drop("ref_query_error_label_genomeid", axis=1, inplace=True)
        ...
        signal_df = signal_df[['block_id', 'label_id', "genome_id", 'motif', 'signal_seg', 'bq', 'label', "mapq", "flag", "pi",
                               "read_bq", "error"]].copy()
        gc.collect()
        ...
        signal_df = signal_df[["block_id", "label_id","genome_id", "label", "motif", "kmer_token", "bq_token", "position_token", "signal_token",
...
def ref_pos_to_query_pos(ref_pos_list, cigar, start_pos, query_len, md_tag, cb_pad = 10):
    ...
    query_pos_dict = {}
    error_arr = np.zeros((query_len,3),dtype=int)
...
def get_label_pos_list(ref, start, cigar, query_len, md_tag, label_df):
    ...
    genome_id_list = label_df["genome_id"].values
    ...
    pos_tuple_list = list(zip(ref_pos_list, query_pos_list, error_list, label_list, genome_id_list))
