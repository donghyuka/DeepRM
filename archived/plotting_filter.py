...
def plot_roc(data_df, outdir, modelname):
    ...
    if "pred_dorado" in data_df.columns:
        ...
        ax.plot([0, 1], [0, 1], color='grey', lw=2, linestyle='--')
    if "pred_dorado_070" in data_df.columns:
        fpr, tpr, _ = roc_curve(data_df["label"], data_df["pred_dorado_070"])
        roc_auc_dorado_070 = auc(fpr, tpr)
        ax.plot(fpr, tpr, lw=3,label=f'Dorado-070 (AUC = {roc_auc_dorado_070:.3f})', color = "tomato", zorder = 1)
        ax.plot([0, 1], [0, 1], color='grey', lw=2, linestyle='--')
...
def plot_pr(data_df, outdir, pr_baseline_level, modelname):
    ...
    if "pred_dorado_070" in data_df.columns:
        ...
        pr_auc_dorado_070 = auc(recall, precision)
        max_f1_dorado_070 = max_f1_score(data_df["label"], data_df["pred_dorado_070"])
        ax.plot(recall, precision, lw=3,label=f'Dorado-070 (AUC = {pr_auc_dorado_070:.3f}, Max F-1 = {max_f1_dorado_070:.3f})', color = "tomato", zorder = 1)
...
def plot_scatter(data_df, outdir, modelname):
    ...
    if "pred_dorado_070" in data_df.columns:
        ...
        ax.set_title(f"Dorado-070, R2 = {r2d:.3f}, rho2 = {rho2d:.3f}")
...
def plot_calibration(data_df, outdir, modelname):
    ...
    if "pred_dorado_070" in data_df.columns:
        ...
        ece = np.abs(p_true - p_pred).mean()
        ax.plot(p_pred, p_true,  lw=3,label=f'Dorado-070 (ECE = {ece:.3f})', color = "tomato", zorder = 1)
    ax.set_xlim([0.0, 1.0])
    ax.set_ylim([0.0, 1.0])
...
def main():
    ...
    dorado_pred = dorado_pred.merge(dorado_pred2, on = "label_id", how = "inner")
    ...
    for idx, data_path in enumerate(args.input):
        ...
        pred_path = f"{args.output}/{modelname}-updatedlabel-dorado-070/pred_read.pkl"
        ...
        if "metadata_list" in data_df_original.columns:
            data_df_original.rename(columns = {"metadata_list": "metadata"}, inplace = True)

        # data_df_original["mis"] = data_df_original["metadata"].apply(lambda x: x["error"][:,0].sum())
        # data_df_original["ins"] = data_df_original["metadata"].apply(lambda x: x["error"][:,1].sum())
        # data_df_original["del"] = data_df_original["metadata"].apply(lambda x: x["error"][:,2].sum())
        # data_df_original["err"] = data_df_original["metadata"].apply(lambda x: x["error"].sum())
        #
        # for filter in range(5):
        #
        #     if filter == 0:
        #         filter_name = "err"
        #         data_df = data_df_original[data_df_original["err"] < 1 ].copy()
        #     elif filter == 1:
        #         filter_name = "mis"
        #         data_df = data_df_original[data_df_original["mis"] < 1 ].copy()
        #     elif filter == 2:
        #         filter_name = "ins"
        #         data_df = data_df_original[data_df_original["ins"] < 1 ].copy()
        #     elif filter == 3:
        #         filter_name = "del"
        #         data_df = data_df_original[data_df_original["del"] < 1 ].copy()
        #     elif filter == 4:
        #         filter_name = "none"
        ...
        if True:
            ...
            for labelpath in label_paths:
                labelname = os.path.basename(labelpath)[:-4]+f"_{filter_name}_depth{args.min_depth}_{args.max_depth}"
