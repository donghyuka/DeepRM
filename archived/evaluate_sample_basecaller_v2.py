...
def inference_worker(rank, args_dict):
    ...
    for source in tqdm.tqdm(data_loader, total=len(data_loader), smoothing = 0):
        ...
        with torch.no_grad():
            ...
            output_canonical = output_basecalling[:,1,:]
            ...
            output_basecalling_a = torch.cat([output_basecalling[:,1,:], output_basecalling[:,5,:]], dim = 1)
            output_basecalling_a = torch.max(output_basecalling_a, dim = 1, keepdim = True)[0]
            output_basecalling = output_basecalling[:,:5,:]
            output_basecalling[:,1,:] = output_basecalling_a
    ...
    data_df = pd.DataFrame({"label_id": id_list, "label": label_list, "pred": pred_list, "block_id": block_id_list,
                            "ce": ce_list, "bce": bce_list, "pred_ca": pred_ca_list})
