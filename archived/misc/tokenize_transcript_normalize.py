...
def segment_normalize_signal(seg_df_path, signal_path_arr, norm_factor, label_df, pid,
    ...
    for signal_path in tqdm.tqdm(signal_path_arr):
        ...
        out_path = f"{seg_df_path}/token_light_v6/{signal_path.split('/')[-1]}"
        ...
        signal_df = signal_df[["label_id", "signal", "bq", "motif"]].copy()
    ...
    out_path = f"{seg_df_path}/token_light_v6/last-{pid}.pkl"
...
def parse_args():
    ...
    parser.add_argument("--toml", "-t", type=str, default="/extdata3/baeklab/Hyeonseo/bin/dorado-0.4.3/model/rna004_130bps_sup@v3.0.1/config.toml", help="Dorado Model TOML file")
...
def main():
    ...
    token_output_path = f"{args.output}/token_light_v6/"
