...
def process_inferece(data_df, updated_label_path, outdir, modelname, baseline_pred, dorado_pred, transformer_pred, ratio, min_depth = 20):

    plot_histogram_read(data_df, outdir, modelname)
    # threshold = gmm_em_lda(data_df["pred"].values)
    threshold_neg = 0.1 ## pre-calculated threshold using GMM-EM-LDA (It is too slow to calculate every time)
    threshold_pos = 0.99
    threshold_dom_neg = 0.3
    threshold_dom_pos = 0.7
    epsilon = 1e-12

    # data_df["pos"] = data_df["block_id"].str.split(":").str[-1].astype(int)
    # data_df = data_df[data_df["pos"] >= 100]
    ...
    data_df_bin["count"] = 1
    data_df_bin.rename(columns = {"pred": "pred_ari"}, inplace = True)
    ...
    if transformer_pred is not None:
        transformer_pred = transformer_pred[["label_id", "pred_geo", "dom"]].copy()
        transformer_pred.rename(columns = {"pred_geo": "pred_transformer", "dom": "dom_transformer"}, inplace = True)
        data_df = data_df.merge(transformer_pred, on = "label_id", how = "inner")
...
def plot_roc(data_df, outdir, modelname):
    ...
    roc_auc = auc(fpr, tpr)
    ax.plot(fpr, tpr, lw=3, label=f'ResNet (AUC = {roc_auc:.3f})', color = "royalblue", zorder = 3)
    if "pred_transformer" in data_df.columns:
        fpr, tpr, _ = roc_curve(data_df["label"], data_df["pred_transformer"])
        roc_auc_transformer = auc(fpr, tpr)
        ax.plot(fpr, tpr, lw=3,label=f'Transformer (AUC = {roc_auc_transformer:.3f})', color = "orchid", zorder = 2)
    ...
    if "pred_dorado" in data_df.columns:
        ...
        ax.plot(fpr, tpr, lw=3,label=f'Dorado (AUC = {roc_auc_dorado:.3f})', color = "forestgreen", zorder = 0)
        ax.plot([0, 1], [0, 1], color='grey', lw=2, linestyle='--')
    ax.set_xlim([0.0, 1.0])
    ax.set_ylim([0.0, 1.0])
...
def plot_pr(data_df, outdir, pr_baseline_level, modelname):
    ...
    precision, recall, scores = precision_recall_curve(data_df["label"], data_df["pred_geo"])
    pr_auc = auc(recall, precision)
    max_f1 = max_f1_score(data_df["label"], data_df["pred_geo"])
    ax.plot(recall, precision, lw=3, label=f'ResNet (AUC = {pr_auc:.3f}, Max F-1 = {max_f1:.3f})', color = "royalblue", zorder = 3)

    if "pred_transformer" in data_df.columns:
        precision, recall, scores = precision_recall_curve(data_df["label"], data_df["pred_transformer"])
        pr_auc_transformer = auc(recall, precision)
        max_f1 = max_f1_score(data_df["label"], data_df["pred_transformer"])
        ax.plot(recall, precision, lw=3,label=f'Transformer (AUC = {pr_auc_transformer:.3f}, Max F-1 = {max_f1:.3f})', color = "orchid", zorder = 2)

    if "pred_m6anet" in data_df.columns:
        precision, recall, scores = precision_recall_curve(data_df["label"], data_df["pred_m6anet"])
    ...
    if "pred_dorado" in data_df.columns:
        precision, recall, scores = precision_recall_curve(data_df["label"], data_df["pred_dorado"])
        pr_auc_dorado = auc(recall, precision)
        max_f1 = max_f1_score(data_df["label"], data_df["pred_dorado"])
        ax.plot(recall, precision, lw=3,label=f'Dorado (AUC = {pr_auc_dorado:.3f}, Max F-1 = {max_f1:.3f})', color = "forestgreen", zorder = 0)
...
def plot_scatter(data_df, outdir, modelname):
    ...
    ax.set_title(f"ResNet, R2 = {r2:.3f}, rho2 = {rho2:.3f}")
    ## polyfit
    ...
    if "pred_transformer" in data_df.columns:
        ax = axes[1]
        ax.scatter(data_df["dom_label"], data_df["dom_transformer"], s = 10, alpha = 0.5, color = "orchid")
        r2d = r2_score(data_df["dom_label"], data_df["dom_transformer"])
        rho2d = spearmanr(data_df["dom_label"], data_df["dom_transformer"])[0] ** 2
        ax.set_title(f"Transformer, R2 = {r2d:.3f}, rho2 = {rho2d:.3f}")
        z = np.polyfit(data_df["dom_label"], data_df["dom_transformer"], 1)
        p = np.poly1d(z)
        x = np.linspace(0, 1, 100)
        ax.plot(x, p(x), color = "grey", linestyle = "--", lw = 3)
...
def plot_histogram_read(data_df, outdir, modelname):
    ...
    plt.rcParams.update({'font.size': 24})
    fig, ax = plt.subplots(figsize = (20,20))
    ax.hist(data_df["pred"], bins = 100, color = "royalblue", label = "ResNet")
...
def plot_histogram_site(data_df, outdir, modelname):
    ...
    plt.rcParams.update({'font.size': 24})
    fig, ax = plt.subplots(figsize = (20,20))
    ax.hist(data_df["pred_geo"], bins = 100, color = "royalblue", label = "ResNet")
    ax.set_xlabel("Prediction")
    ax.set_ylabel("Count")
    ax.set_title(f"{modelname}")
    ax.legend()
...
def plot_calibration(data_df, outdir, modelname):
    ...
    ax.plot(p_pred, p_true, lw=3, label=f'ResNet (ECE = {ece:.3f})', color = "royalblue", zorder = 3)
    if "pred_m6anet" in data_df.columns:
    ...
    if "pred_transformer" in data_df.columns:
        p_true, p_pred = calibration_curve(data_df["label"], data_df["pred_transformer"], n_bins = 10)
        ece = np.abs(p_true - p_pred).mean()
        ax.plot(p_pred, p_true,  lw=3,label=f'Transformer (ECE = {ece:.3f})', color = "orchid", zorder = 0)
    ax.set_xlim([0.0, 1.0])
    ax.set_ylim([0.0, 1.0])
...
def main():
    ...
    transformer_pred = pd.read_csv("/extdata4/baeklab/Hyeonseo/m6A/inference/plot/BERMUDA-Proto-v19-20240404-092850-26-221000-baeklab_v2_depth20_drach-updatedlabel-dorado/BaeklabV2_GP3.depth20.twm6astrict.notsampled.drach/pos0.90_bce0.10/BERMUDA-Proto-v19-20240404-092850-26-221000-baeklab_v2_depth20_drach.tsv",
                                      sep = "\t")
    ...
    for idx, data_path in enumerate(args.input):
        ...
        for labelpath in label_paths:
            ...
            data_df = process_inferece(data_df_original.copy(), labelpath, outdir, modelname, baseline_pred, dorado_pred,
                                       transformer_pred, args.ratio, args.depth)
