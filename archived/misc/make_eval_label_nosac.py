...
def get_data_df(gp_cutoff):
    ...
    data_df = data_df[data_df["transcript_coordinate"] >= 100].copy()
    data_df["id"] = data_df["NMID"] + ":" + data_df["transcript_coordinate"].astype(str)
    data_df["GLORI"] = data_df["DoM"] > 0.1
    data_df["validated"] = data_df[["GLORI","MICLIP2","M6ACE"]].astype(int).sum(axis=1)
...
def sample_eval_data(datid_df, seed = None, ratio = 10, drach = False, non_drach = False):
    if drach:
...
def main(depth_cutoff = 20, gp_cutoff=4,
    ...
    label_name = f"BaeklabV2_GP{gp_cutoff}.nosac.gloricut.tss.depth{depth_cutoff}"
    if filter_adjacent:
    ...
    datid_df_s = sample_eval_data(datid_df.copy(), non_drach = True, ratio = False)
    datid_df_s.to_csv(f"/extdata4/baeklab/Hyeonseo/m6A/runs/exp_MRNA/ON0090/ON0090/label/{label_name}.notsampled.nondrach.tsv", sep='\t', index=False)
