...
def sample_and_save_df(in_path_list, out_path, ncpu, label, chunk, max_token_len,
    ...
    for set_name in set_split_dict:
        remainder_df_dict[set_name] = man.list()
    ...
    for pid in range(ncpu):
        ...
        proc = mp.Process(target=sample_and_save_df_worker, args=(in_file_list[pid], out_path, label_str,
                                                                  set_split_dict, pid_str, chunk, label,
        ...
        if len(remainder_df_list) > 0:
            ...
            for chunk_idx in range(0, len(remainder_df) // chunk + 1):
                ...
                if len(chunk_df) == chunk:
                    ...
                    save_path = f"{out_path}/{set_name}/{label_str}/{file_name}"
...
def sample_and_save_df_worker(in_file_list, out_path, label_str, set_split_dict, pid_str,
                              chunk, label, max_token_len, remainder_df_dict, id_digit, shuffle):
    ...
    for df_path in tqdm.tqdm(in_file_list):
        ...
        data_df["label"] = label
        data_df["token_len"] = data_df["signal_token"].apply(lambda x: len(x))

        data_df = data_df[data_df["token_len"] <= max_token_len]
        ...
        set_idx = np.cumsum([0] + [int(np.floor(len(data_df) * split_ratio)) for split_ratio in set_split_dict.values()])

        for idx, set_name in enumerate(set_split_dict):
            set_idx_start = set_idx[idx]
            set_idx_end = set_idx[idx+1]
            set_df = data_df.iloc[set_idx_start:set_idx_end].reset_index(drop=True)
...
def main():
    ...
    if args.pos_path is not None:
        sample_and_save_df(args.pos_path, args.out_path, args.cpu, label = 1,
                           chunk = args.chunk, max_token_len = args.max_token_len)
