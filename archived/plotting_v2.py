...
def process_inferece(data_df, updated_label_path, outdir, modelname, baseline_pred, dorado_pred, ratio, min_depth = 10):
    ...
    data_df_bin = data_df_bin[data_df_bin["bce"] < 0.01].copy()
    ...
    data_df_bin = data_df_bin.groupby("label_id").agg({"pred_ari": "mean", "pred_geo": mean_without_max,"count": "sum"}).reset_index()
