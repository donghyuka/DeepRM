...
from train.dataloader_intern import load_dataset, NanoporeDataLoader
...
def parse_args():
    ...
    parser.add_argument("--tb", dest = "tb_path", type=str, required=True)
    ...
    parser.add_argument("--tcp_port", type=str, default="12355")
...
def setup_ddp(rank,world_size,gpu_id, tcp_port):
    os.environ['MASTER_ADDR'] = 'localhost'
    os.environ['MASTER_PORT'] = tcp_port
...
def main_worker(rank, args_dict):
    ...
    setup_ddp(rank=rank, world_size=args_dict["num_gpu"], gpu_id=gpu_id, tcp_port=args_dict["tcp_port"])

    #########################################
    ## Your model is called here.
    ## You need to implement NanoporeModel in a separate file.
    #########################################

    NanoporeModel = importlib.import_module(f"model.{args_dict['model_type']}").NanoporeModel
    model = NanoporeModel(**args_dict)
    ...
    #########################################
    ## Define your metric functions here (as a dictionary).
    #########################################
