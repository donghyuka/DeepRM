...
class Trainer:
    def __init__(
        ...
        self.signal_stride = 6
        self.aug_fraction = 0.05
    ...
    def _get_aug_list(self):
        ...
        movmag = partial(aug.moving_magnitude_warp, fraction=fraction, min_sigma=0.2, max_sigma=0.8, n_knots = 60)
        ## WINDOWED TIME WARP
        winwarp = partial(aug.window_warp, fraction=fraction, min_window_ratio = 0.05, max_window_ratio = 0.1,
                          min_window_count = 5, max_window_count = 10, sigma = 0.5)
        ## TIME WARP
        timewarp = partial(aug.time_warp, fraction=fraction, min_sigma=0.1, max_sigma=0.2, n_knots = 10)
        ## GAUSSIAN JITTER
        jitter = partial(aug.jitter, fraction=fraction, min_sigma=0.1, max_sigma=0.2)
        ## SPIKE NOISE
        spike = partial(aug.jitter, fraction=fraction, min_sigma=1.0, max_sigma=2.0, dropout = 0.95)
        ## STEP NOISE
        step = partial(aug.step, fraction=fraction, min_sigma=0.1, max_sigma=0.2, dropout = 0.95)
        ## SLOPE NOISE
        slope = partial(aug.slope, fraction=fraction, magnitude = 1.0,)
        ## DRIFT NOISE
        drift = partial(aug.drift, fraction=fraction, min_sigma=0.1, max_sigma=0.5, n_knots = 40)
        aug_list = [movmag, winwarp, timewarp, jitter, spike, step, slope, drift]
        return aug_list
    ...
    def _augment_signal(self, signal, pad_mask, shuffle_order = True):
        ...
        for aug_func in aug_list:
            signal = aug_func(signal, pad_mask)
    ...
    def _feed_model(self, source, target):
        ...
        signal_pad_mask = src_pad_mask.repeat_interleave(self.signal_stride, dim = 1)
        src_signal = self._augment_signal(src_signal, signal_pad_mask, shuffle_order = True)
...
def prepare_dataloader(data_path, batch_size, eval_batch_size, disk_shard_size, rank, num_replicas, buffer_size,
                       read_every, seed, class_ratio, prefetch, pin_memory, soft_label, num_workers):
