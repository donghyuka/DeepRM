...
from train.dataloader_dwell_bq_v3 import load_dataset, NanoporeDataLoader
...
def parse_args():
    ...
    parser.add_argument("--output", type=str, default="/extdata4/baeklab/Hyeonseo/m6A/eval")
...
class Trainer:
    ...
    def _run_epoch(self):
        ...
        dist.barrier()
        outputs, targets = self._run_eval()
        outputs = outputs.cpu().detach().numpy()
        targets = targets.cpu().detach().numpy()
        eval_df = pd.DataFrame({"output":outputs, "target":targets})
        eval_df.to_pickle(f"{self.checkpoint_path}/eval_df_{self.rank}.pkl")
        return None
...
def main_worker(rank, args_dict):
    ...
    val_loader = prepare_dataloader(args_dict["data"], args_dict["batch_size"], args_dict["eval_batch_size"],
    ...
    trainer = Trainer(rank, gpu_id, model, val_loader, optimizer, scheduler, loss_func, args_dict["grad_clip"], metric_func_dict,
