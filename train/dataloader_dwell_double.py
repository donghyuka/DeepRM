...
class BinaryClassDatasetIterator:
    def __init__(self, pos_file_paths, neg_file_paths, disk_shard_size, shuffle_buffer_size,
        ...
        self.keys = ["segment_len_arr", "signal_token", "kmer_token", "dwell_motor_token", "dwell_pore_token", "bq_token"]
...
def pad_collate(batch, pad_to, signal_stride, kmer_size, trim = 2):
    ...
    src_dwell_pore = np.stack(dwell_pore_token_list)[:,4:-4]
    src_signal = torch.tensor(np.stack(signal_token_list), dtype=torch.float32)
    src_dwell_bq = torch.tensor(np.stack([src_dwell_motor, src_dwell_pore], axis = -1), dtype=torch.float32)
