...
class BinaryClassDatasetIterator:
    def __init__(self, pos_file_paths, neg_file_paths, buffer_size, read_every = 1000, shuffle = True, class_ratio = 1,
        ...
        self.columns = ["seq_token", "bq_token", "signal_token", "move_token", "target_mask"]
    ...
    def nanopore_row_to_tensor(self, row, class_idx):
        ...
        kmer_token = torch.tensor(row[0], dtype=torch.float)
        bq_token = torch.tensor(row[1], dtype=torch.float)
        signal_token = torch.tensor(row[2], dtype=torch.float)
        move_token = torch.tensor(row[3], dtype=torch.float)
...
def load_dataset(pos_data_path, neg_data_path, batch_size,
                 ...
                 pad_to = 1000, bq_clip = 40, class_ratio = 1, prefetch_factor = 512, pin_memory=True, soft_label = False):
...
def pad_collate(batch, pad_to, bq_clip):
    ...
    token_name_list = ["kmer_token", "bq_token", "signal_token",
