...
from train.dataloader_dwell_npz import load_dataset, NanoporeDataLoader
...
def parse_args():
    ...
    if args.name is None:
        args.name = f"AIRNA-DW-{args.model.split('_')[-1]}-{strfttime}"
...
class Trainer:
    ...
    def _feed_model(self, source, target):
        ...
        src_dwell = source["dwell_token"].to(self.gpu_id)

        output = self.model(src_kmer, src_signal, src_seg_len, src_dwell)
