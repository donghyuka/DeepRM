...
from train.dataloader_bpp import load_dataset, NanoporeDataLoader
...
class Trainer:
    ...
    def _feed_model(self, source, target):
        ...
        src_structure = source["structure_token"].to(self.gpu_id)

        if not self.no_bq:
            src_bq = source["bq_token"].to(self.gpu_id)
            output = self.model(src_kmer, src_signal, src_seg_len, src_bq, src_structure)

        else:
            output = self.model(src_kmer, src_signal, src_seg_len, src_structure)
