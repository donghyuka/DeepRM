...
def sample_and_save_df_worker(ncpu, pid, in_file_list, out_path, label_str, set_split_dict,
    ...
    for df_idx, df_path in tqdm.tqdm(enumerate(in_file_list), desc=f"Saving {label_str} data", total=len(in_file_list)):
        ...
        orig_df["signal_token"] = orig_df["signal_token"].apply(lambda x: pad_signal(x,1224))
...
def chunk_save_df(ncpu, pid, file_id, set_df, out_path, label_str, set_name, buffer_dict, chunk, score_name, id_digit):
    for chunk_idx in range(0, len(set_df) // chunk + 1):
        ...
        if len(chunk_df) == chunk:
            save_path = f"{out_path}/score-{score_name}/{set_name}/{label_str}/{str(out_df_id).zfill(id_digit)}.npz"
...
def save_npz(save_path, df):
    ...
    dwell_token = np.stack(df["dwell_token"].values)
    np.savez_compressed(save_path, segment_len_arr=segment_len_arr, signal_token=signal_token, kmer_token=kmer_token, dwell_token=dwell_token)
    return None
