...
def parse_args():
    ...
    parser.add_argument("--rename", "-r", type=int, default="", help="rename input directory")
...
def main():
    ...
    cmd = f"{args.dorado}/bin/dorado basecaller -x {args.gpu} -b {args.batch} --min-qscore {args.qcut} --emit-moves --estimate-poly-a {dorado_model_path} {pod5_path} > {bam_path}"
    print(cmd)
    os.system(cmd)
