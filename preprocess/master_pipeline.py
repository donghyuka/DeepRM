...
def parse_args():
    ...
    parser.add_argument("--in", "-i", dest = "pod5", type=str, required=True, help="POD5 Input directory")
...
def main():
    ...
    Steps:
        ...
        3. Run Signal Segmentation & Normalization.
    ...
    if 2 in args.step:
        ...
        cmd = f"python -m preprocess.dag_extract_cb --cpu {args.cpu} --input {bam_path} --output {block_df_path} --rbq {args.qcut}"
        print_run(cmd,args.dry)
    ...
    if 3 in args.step:
        ...
        printmessage(f"[Step 3/3] Running Signal Segmentation & Normalization")

        cmd = f"python -m preprocess.segment_normalize_signal --cpu {args.cpu} --pod5 {args.pod5} --bam {bam_path} --block {block_df_path} --output {block_path}"
        print_run(cmd,args.dry)
