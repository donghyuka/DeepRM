...
def segmented_signal_to_block(signal_segmented, segment_len_arr, kmer, sampling, sig_window, pad_to):
    try:
        ...
        padding = pad_to * sampling - len(signal_segmented)
        if padding > 0:
...
def segment_normalize_signal(seg_df_path, signal_path_arr, norm_factor, kmer = 5, cb_len = 21, sampling = 6,
    ...
    for signal_path in tqdm.tqdm(signal_path_arr, total=len(signal_path_arr), desc="Segmenting and Tokenizing Signals"):
        ...
        out_path = f"{seg_df_path}/token_dwell_normalised/{file_id}"
        ...
        signal_df["dwell_token"] = signal_df.apply(lambda x: x["dwell_token"][(x["start_pos"]+dwell_shift+trim):(x["end_pos"]+dwell_shift-trim)], axis=1)
        ...
        signal_df = signal_df[["segment_len_arr", "signal", "motif", "dwell_token"]].copy()
        signal_df.rename(columns={"motif": "kmer_token", "signal": "signal_token"}, inplace=True)
...
def main():
    ...
    for output in args.output:
        ...
        token_output_path = f"{output}/token_dwell_normalised/"
