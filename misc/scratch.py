...
path = "/extdata4/baeklab/Hyeonseo/m6A/runs/exp_MRNA/ON0090/ON0090/result/dorado_m6a/dorado_m6a_basecalled.pileup.pkl"
df = pd.read_pickle(path)
df["NMID"] = df["label_id"].str.split(":").str[0]
df["NMID"] = df["NMID"].map(convert_dict)
df["pos"] = df["label_id"].str.split(":").str[1]
df["label_id"] = df["NMID"] + ":" + df["pos"]
df.drop(["NMID", "pos"], axis = 1, inplace = True)
df.to_pickle("/extdata4/baeklab/Hyeonseo/m6A/runs/exp_MRNA/ON0090/ON0090/result/dorado_m6a/dorado_m6a_basecalled.pileup.reformatted.pkl")