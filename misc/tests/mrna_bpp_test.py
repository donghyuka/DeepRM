...
def get_bpp(pid, output, coordinate_dict, fasta_dict, pad):
    bpp_dict = {}
    for transcript_id, coordinates in tqdm(coordinate_dict.items(), total = len(coordinate_dict), desc = "Calculating BPP"):
        try:
            ...
            fc.pf()
            bpp = np.array(fc.bpp())
            bpp = (bpp + bpp.T).sum(axis = 0)

            for coordinate in coordinates:
                centre = coordinate.split(":")[1]
                centre = int(centre)
                if centre - pad < 0 or centre + pad >= seq_len:
                    bpp_dict[coordinate] = None
                else:
                    bpp = bpp[centre-pad:centre+pad+1].mean()
                    bpp_dict[coordinate] = bppq
        except:
            for coordinate in coordinates:
                bpp_dict[coordinate] = None

    with open(f"{output}/temp/bpp_{pid}.pkl", "wb") as f:
        pickle.dump(bpp_dict, f)
...
def main():
    ...
    for i in range(args.threads):
        proc = mp.Process(target = get_bpp, args = (i, args.output, split_coordinate_dict[i], split_fasta_dict[i], args.pad))
...
def main_2():
    ...
    for group, path in group_dict.items():
        ...
        mfe_dict = {}
        for i in tqdm(range(args.threads), desc = "Merging"):
            with open(f"{args.output}/temp/bpp_{i}.pkl", "rb") as f:
                data = pickle.load(f)
            mfe_dict.update(data)
        with open(f"{args.output}/bpp_pad{args.pad}_{group}.pkl", "wb") as f:
            pickle.dump(mfe_dict, f)
...
def plot(path, pad):
    ...
    path_dict = {"FN": f"{path}/bpp_pad{pad}_FN.pkl",
                 "FP": f"{path}/bpp_pad{pad}_FP.pkl",
                 "TP": f"{path}/bpp_pad{pad}_TP.pkl"}
    mfe_dict = {}
    ...
    for group, subpath in path_dict.items():
        ...
        print(f"{group} mean: {np.mean(val)}")
        print(f"{group} std: {np.std(val)}")
        print(f"{group} median: {np.median(val)}")
        print()

    ## Plot KDE
    fig, ax = plt.subplots(1, 1, figsize = (20, 10))
    colours = ["royalblue", "tomato", "forestgreen"]
    bpp_list = [mfe_dict[x] for x in mfe_dict.keys()]
    df = pd.DataFrame({"BPP": np.concatenate(bpp_list),
                       "group": np.concatenate([[x]*len(mfe_dict[x]) for x in mfe_dict.keys()])})
    df["group"] = df["group"].apply(lambda x: f"{x} (n={len(mfe_dict[x])})")
    sns.kdeplot(data = df, x = "BPP", hue = "group", fill = False, palette = colours,
                common_norm = False, common_grid = True, legend = True, ax = ax)
    ax.set_title(f"BPP Distribution (window = {2*pad+1})")
    ax.set_xlabel("BPP")
    ax.set_ylabel("Density")

    fig.savefig(f"{path}/bpp_pad{pad}.png")
...
if __name__ == "__main__":
    # main()
    # main_2()
    plot("/extdata4/baeklab/Hyeonseo/m6A/postprocess/bpp", 10)