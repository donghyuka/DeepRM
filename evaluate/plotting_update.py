...
def parse_args():
    ...
    parser.add_argument("--label", "-l", type=str, default="/extdata4/baeklab/Hyeonseo/m6A/runs/exp_MRNA/ON0090/ON0090/label/Baeklab_GP3.depth20.adj10.twm6a.notsampled.tsv", help="Label path")
...
def process_inferece(data_df, updated_label_path, outdir, modelname, baseline_pred, ratio):
    ...
    data_df["pred_geo"] = data_df["pred_ari"]
    ## groupby label_id and remove max
    data_df = data_df.groupby("label_id").agg({"pred_ari": "mean", "pred_geo": np.prod,
                                               "dom": "mean", "count": "sum"}).reset_index()

    data_df = data_df.merge(baseline_pred, on = "label_id", how = "inner")
    data_df.fillna(0, inplace = True)
    #
    # lenient_label_df = pd.read_csv("/extdata4/baeklab/Hyeonseo/m6A/runs/exp_MRNA/ON0090/ON0090/label/BaeklabV2_GP3.depth20.notsampled.drach.tsv", sep = "\t")
    # lenient_label_df = lenient_label_df[["id", "label", "m6A_level"]]
    # lenient_label_df.rename(columns = {"id": "label_id","m6A_level": "dom_label"}, inplace = True)
    # lenient_data_df = data_df.merge(lenient_label_df, on = "label_id", how = "inner")
    # plot_scatter_lenient(lenient_data_df, outdir, modelname)
...
def plot_scatter(data_df, outdir, modelname):
    ...
    ax.set_title(f"m6Anet, R2 = {r2b:.3f}")
...
def plot_scatter_lenient(data_df, outdir, modelname):
    plt.rcParams.update({'font.size': 24})
    fig, axes = plt.subplots(1,2, figsize = (40,20))
    ## ax0: dom_label vs. dom
    ## ax1: dom_label vs. dom_m6anet
    ...
    for ax in axes:
        ...
        ax.set_ylabel('Predicted DoM')
    fig.suptitle(f"{modelname}")
    plt.savefig(f"{outdir}/scatter_lenient.png")
    plt.close()
    return r2
...
def main():
    ...
    for idx, data_path in enumerate(args.input):
        ...
        for labelpath in label_paths:
            ...
            data_df = process_inferece(data_df_original.copy(), labelpath, outdir, modelname, baseline_pred, args.ratio)
