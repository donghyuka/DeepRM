...
class NanoporeDatasetIterator:
    ...
    def _read_df(self):
        ...
        df["structure"] = df["motif"].apply(lambda x: RNA.fold(x)[1])
        df["kmer_token"] = df["motif"].apply(lambda x: np.array(list(x)))
        df["bq_token"] = df["bq"].apply(lambda x: x[self.trim:-self.trim])
        df = df[["kmer_token", "signal_token", "bq_token","structure_token","segment_len_arr","label_id","block_id"]][df["signal_token"].notnull()].copy()
...
def pad_collate(batch, pad_to, signal_stride, kmer_size):
    ...
    for source in batch:
        ...
        structure_list.append(source[3])
        segment_len_list.append(source[4])
        label_id_list.append(source[5])
        block_id_list.append(source[6])
    ...
    src_structure = torch.tensor(np.stack(structure_list), dtype=torch.float)
    ...
    source["structure_token"] = src_structure
