...
class NanoporeDatasetIterator:
    ...
    def _read_df(self):
        ...
        df["segment_len_arr"] = df["signal"].apply(lambda x: create_segment_len_arr(x, sampling))
        df["signal_token"] = df.apply(lambda x: segmented_signal_to_block(x["signal"], x["segment_len_arr"], kmer, sampling, sig_window), axis=1)
        ...
        df["metadata"] = df.apply(lambda x: {"motif": x["motif"], "mapq": x["mapq"], "flag": x["flag"], "pi": x["pi"],
                                                "read_bq": x["read_bq"], "block_bq": x["block_bq"], "base_bq": x["base_bq"],
                                                "error": x["error"], "query_pos": x["query_pos"], "query_len": x["query_len"],
                                                "left_soft_clip": x["left_soft_clip"]}, axis=1)

        df = df[["kmer_token", "bq_token", "signal_token", "move_token", "target_mask","label_id","block_id", "label", "dom", "metadata"]][df["signal_token"].notnull()].copy()
    ...
    def nanopore_row_to_tensor(self, row):
        ...
        return_dict = {"kmer_token": kmer_token, "bq_token": bq_token,
                       ...
                       "target_mask": target_mask, "label_id": label_id, "label": label, "dom": dom,
...
def pad_collate(batch, pad_to, bq_clip):
    ...
    source["metadata"] = [item[0]["metadata"] for item in batch]
    source["block_id"] = [item[0]["block_id"] for item in batch]
    source["dom"] = [item[0]["dom"] for item in batch]
