...
def get_data_df(gp_cutoff, data_path):
    ...
    data_df["label"] = data_df["validated"] >= gp_cutoff
    data_df["label"] = data_df["label"].astype(int)
    data_df["label"] = 2 * data_df["label"] - 1
    data_df = data_df[['id', 'NMID', 'DoM', 'label']]
    data_df.rename({"DoM":"m6A_level","NMID":"nmid"}, axis=1, inplace=True)
    return data_df

def make_label_df(return_list, data_df_list, depth_df_list, refflat_df_list,
                  filter_adjacent, adjacent_distance, adjacent_strict,
                  filter_no_m6a , no_m6a_strict):
    local_collect = []

    for data_df, depth_df, refflat_row in zip(data_df_list, depth_df_list, refflat_df_list):
        if len(depth_df) == 0:
            continue
        elif data_df is None:
            data_df = pd.DataFrame(columns=['id', 'nmid', 'm6A_level', 'label'])
        elif len(data_df) == 0:
            data_df = pd.DataFrame(columns=['id', 'nmid', 'm6A_level', 'label'])
        ...
        depth_df["genome_id"] = depth_df.apply(lambda row: transcript_to_chromosomal_coordinate(row["pos"], chrom,exon_starts,exon_ends,strand,exon_cumsum), axis=1)
        depth_df = depth_df.dropna()
        local_collect.append(depth_df)

    if len(local_collect) == 0:
        return None

    depth_df = pd.concat(local_collect)
...
def main():
    ...
    refflat_df_split = [[] for i in range(args.cpu)]
    ...
    for idx, group in tqdm.tqdm(enumerate(groups), desc="Splitting depth_df", total=len_groups):
        ...
        try:
            refflat_df_split[split_idx].append(refflat_df.loc[nmid])
        except KeyError:
            continue
    ...
    for depth_df, data_df, refflat_df in zip(depth_df_split, data_df_split, refflat_df_split):
        proc = mp.Process(target=make_label_df, args=(return_list, data_df, depth_df, refflat_df,
                                                      filter_adjacent, adjacent_distance, adjacent_strict,
                                                      filter_no_m6a, no_m6a_strict))
