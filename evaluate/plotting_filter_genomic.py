...
def parse_args():
    ...
    parser.add_argument("--dorado", "-d", type=str, default="/extdata4/baeklab/Hyeonseo/m6A/runs/exp_MRNA/ON0090/ON0090/result/dorado_m6a/dorado_m6a_basecalled.pileup.genomic.pkl", help="Dorado path")
    parser.add_argument("--label", "-l", type=str, default="/extdata4/baeklab/Hyeonseo/m6A/runs/exp_MRNA/ON0090/ON0090/label/BaeklabV3.Genome.GP3.depth1_None.twm6astrict.notsampled.drach.tsv", help="Label path")
    ...
    parser.add_argument("--max_depth", "-x", type=int, default=0, help="Maximum depth")
...
def process_dorado_inferece(data_path):
    ...
    data_df = data_df.groupby("genome_id").agg({"depth_m6a": "sum", "depth_ca": "sum"}).reset_index()
    data_df["dorado_count"] = data_df["depth_m6a"] + data_df["depth_ca"]
    data_df["pred_dorado"] = data_df["depth_m6a"] / data_df["dorado_count"]
    data_df.drop(columns = ["depth_m6a", "depth_ca"], inplace = True)
    return data_df
...
def process_inferece(data_df, updated_label_path, outdir, modelname, baseline_pred, dorado_pred, ratio, min_depth, max_depth):
    ...
    data_df["pred_geo"] = np.log10(1 - np.clip(data_df["pred_ari"].to_numpy(), 0.0, 1 - epsilon))
    ...
    data_df = data_df.groupby("genome_id").agg({"count": "sum", }).reset_index()
    data_df_bin = data_df_bin.groupby("genome_id").agg({"pred_ari": "mean", "pred_geo": "mean", }).reset_index()
    data_df_dom = data_df_dom.groupby("genome_id").agg({"dom": "mean"}).reset_index()
    data_df = data_df.merge(data_df_bin, on = "genome_id", how = "inner")
    data_df = data_df.merge(data_df_dom, on = "genome_id", how = "inner")
    data_df["count"] = data_df["count"].fillna(0)
    data_df["pred_geo"] = (1 - 10**np.clip(data_df["pred_geo"].to_numpy(), None, 0))

    if baseline_pred is not None:
        data_df = data_df.merge(baseline_pred, on = "genome_id", how = "inner")
    if dorado_pred is not None:
        data_df = data_df.merge(dorado_pred, on = "genome_id", how = "inner")

    updated_label_df = pd.read_csv(updated_label_path, sep = "\t")
    updated_label_df = updated_label_df[["genome_id", "label", "m6A_level"]]
    updated_label_df.rename(columns = {"m6A_level": "dom_label"}, inplace = True)
    data_df = data_df.merge(updated_label_df, on = "genome_id", how = "inner")
    if min_depth > 0:
    ...
    data_df = data_df.drop_duplicates(subset = "genome_id", keep = "first")
...
def plot_scatter(data_df, outdir, modelname):
    ...
    ax.set_title(f"Transformer-BIN, R2 = {r2:.3f}, rho2 = {rho2:.3f}")
    ## polyfit
    ...
    ax = axes[1]
    ax.scatter(data_df["dom_label"], data_df["pred_ari"], s = 10, alpha = 0.5, color = "tomato")
    r2 = r2_score(data_df["dom_label"], data_df["pred_ari"])
    rho2 = spearmanr(data_df["dom_label"], data_df["pred_ari"])[0] ** 2
    ax.set_title(f"Transformer-ARI, R2 = {r2:.3f}, rho2 = {rho2:.3f}")
    ## polyfit
    z = np.polyfit(data_df["dom_label"], data_df["pred_ari"], 1)
    p = np.poly1d(z)
    x = np.linspace(0, 1, 100)
    ax.plot(x, p(x), color = "grey", linestyle = "--", lw = 3)
...
def main():
    ...
    for idx, data_path in enumerate(args.input):
        ...
        # data_df_original["bq"] = data_df_original["metadata"].apply(lambda x: x["read_bq"])
        # data_df_original["all_error"] = data_df_original["metadata"].apply(lambda x: x["error"].sum())
        # data_df_original["cent_error"] = data_df_original["metadata"].apply(lambda x: x["error"][0].sum())
        # data_df_original["pi"] = data_df_original["metadata"].apply(lambda x: x["pi"])
        # data_df_original["flag"] = data_df_original["metadata"].apply(lambda x: x["flag"])
        # data_df_original["mapq"] = data_df_original["metadata"].apply(lambda x: x["mapq"])
        # #
        # for filter in range(9):
        #
        #     if filter == 0:
        #         filter_name = "bq_7"
        #         data_df = data_df_original[data_df_original["bq"] >= 7].copy()
        #     elif filter == 1:
        #         filter_name = "error_all"
        #         data_df = data_df_original[data_df_original["all_error"] == 0].copy()
        #     elif filter == 2:
        #         filter_name = "error_cent"
        #         data_df = data_df_original[data_df_original["cent_error"] == 0].copy()
        #     elif filter == 3:
        #         filter_name = "flag"
        #         data_df = data_df_original[data_df_original["flag"] == 0].copy()
        #     elif filter == 4:
        #         filter_name = "mapq_10"
        #         data_df = data_df_original[data_df_original["mapq"] >= 10].copy()
        #     elif filter == 5:
        #         filter_name = "mapq_30"
        #         data_df = data_df_original[data_df_original["mapq"] >= 30].copy()
        #     elif filter == 6:
        #         filter_name = "mapq_60"
        #         data_df = data_df_original[data_df_original["mapq"] >= 60].copy()
        #     elif filter == 7:
        #         filter_name = "pi"
        #         data_df = data_df_original[data_df_original["pi"] == 0].copy()
        #     elif filter == 8:
        ...
        if True:
            ...
            data_df = data_df_original.dropna().copy()
            ...
            for labelpath in label_paths:
                labelname = os.path.basename(labelpath)[:-4]+"_"+filter_name+"_depth"+str(args.min_depth)+"_"+str(args.max_depth)
                outdir = os.path.join(args.output, os.path.dirname(pred_path), labelname)
                os.makedirs(outdir, exist_ok = True)
                data_df = process_inferece(data_df, labelpath, outdir, modelname, baseline_pred, dorado_pred,
