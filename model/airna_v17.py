...
class TransformerModel(nn.Module):
    ...
    def forward(self, src_kmer: Tensor, src_signal: Tensor, src_seg_len: Tensor, src_bq: Tensor) -> Tensor:
        with torch.no_grad():
            src_signal = self.interp1d(
                torch.arange(src_signal.shape[1], device=src_signal.device).unsqueeze(0).repeat(src_signal.shape[0], 1),
                src_signal,
                (src_seg_len / self.unit_size).repeat_interleave(self.unit_size * self.signal_stride, dim=1).cumsum(
                    dim=1)).unfold(1, self.signal_stride * self.kmer_size, self.signal_stride)
            src_kmer = ((((src_kmer - 65).clip(None, 8) % 5).unfold(1, self.kmer_size, 1) * (
                        4 ** torch.arange(self.kmer_size, device=src_kmer.device, dtype=torch.int)).unsqueeze(
                0).unsqueeze(0)).sum(dim=-1) + 1)
        ...
        output = self.transformer_decoder_1(tgt=target, memory=output, tgt_mask=None, memory_mask=None,
                                            tgt_key_padding_mask=None, memory_key_padding_mask=None)
        target = self.bq_embedding(src_bq) + self.pe_seq(src_bq.size(0))
