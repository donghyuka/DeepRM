...
class TransformerModel(nn.Module):
    ...
    def __init__(self, d_model: int, n_heads: int, d_ff: int,
        ...
        self.kmer_embedding = nn.Embedding(4**kmer_size+1, d_model//6)
        self.signal_embedding = nn.Linear(signal_size, d_model//6)
        self.spectrogram_embedding = nn.Linear(spectrogram_size, d_model//6)
        self.bq_embedding = nn.Embedding(max_bq+1, d_model//6)
        self.pos_encoding = PositionalEncoding(d_model//6)
        self.move_embedding = nn.Embedding(block_len+1, d_model//6)
    ...
    def forward(self, src_kmer: Tensor, src_signal: Tensor, src_spectrogram: Tensor, src_bq: Tensor, src_move: Tensor,
        ...
        final_embedding = torch.concat([kmer_embedding, signal_embedding, spectrogram_embedding, bq_embedding,
                                        pos_encoding, move_embedding], dim = -1)
