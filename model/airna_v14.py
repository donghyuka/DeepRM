...
class TransformerModel(nn.Module):
    ...
    def __init__(self, d_model: int, n_heads: int, d_ff: int,
        ...
        self.abs_pe_sig = PositionalEncoding(d_model//2, seq_len)
        self.abs_pe_seq = PositionalEncoding(d_model, block_len)
        self.rel_pe_sig    = RelativePositionalEncoding(d_model//2, seq_len)
    ...
    def forward(self, src_kmer: Tensor, src_signal: Tensor, src_seg_len: Tensor, src_bq: Tensor) -> Tensor:
        ...
        with torch.no_grad():
            ...
            src_kmer = (src_kmer * (4**torch.arange(self.kmer_size, device = src_kmer.device)).unsqueeze(0).unsqueeze(0)).sum(dim = -1) + 1
        ...
        abs_pe_sig =  self.abs_pe_sig(src_signal.size(0))
        abs_pe_seq =  self.abs_pe_seq(src_kmer.size(0))
        rel_pe_sig =  self.rel_pe_sig(src_seg_len)

        memory = self.signal_embedding(src_signal) + torch.cat([abs_pe_sig, rel_pe_sig], dim = 2)
        memory = self.transformer_encoder(src=memory, mask = None, src_key_padding_mask = src_pad_mask)
        target = self.kmer_embedding(src_kmer) + abs_pe_seq
        output = self.transformer_decoder_1(tgt=target, memory=memory, tgt_mask = None, memory_mask = None,
                                            tgt_key_padding_mask = None, memory_key_padding_mask = src_pad_mask)
        output = output + self.bq_embedding(src_bq)
        output = self.transformer_decoder_2(tgt=output, memory=memory, tgt_mask = None, memory_mask = None,
                                            tgt_key_padding_mask = None, memory_key_padding_mask = src_pad_mask)
...
class RelativePositionalEncoding(nn.Module):
    ...
    def forward(self, src_seg_len:Tensor) -> Tensor:
        ...
        div_term = torch.exp(torch.arange(0, self.d_model, 2, device=src_seg_len.device) * (-math.log(10000.0) / self.d_model)).unsqueeze(0).unsqueeze(0)
        pe = torch.zeros(src_seg_len.shape[0], self.seq_len, self.d_model, device=src_seg_len.device)
        pe[:, :, 0::2] = torch.sin(position * div_term)
        pe[:, :, 1::2] = torch.cos(position * div_term)
        return pe
