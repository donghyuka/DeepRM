...
class TransformerModel(nn.Module):
    ...
    def __init__(self, d_model: int, n_heads: int, d_ff: int,
        ...
        self.regression_head = RegressionHead(d_model, d_model//4, lin_act, lin_depth, lin_dropout, seq_len)
        self.regression_head = nn.SyncBatchNorm.convert_sync_batchnorm(self.regression_head)
        self.basecalling_head = nn.Sequential(nn.Linear(d_model//4, d_model//4), nn.GELU(),
                                              nn.Linear(d_model//4, 6))

        self.modification_head = nn.Sequential(nn.Linear(d_model//4, d_model//4), nn.GELU(),
    ...
    def init_weights(self, initrange = 0.1):
        ...
        for module in self.basecalling_head:
            if isinstance(module, nn.Linear):
                module.weight.data.uniform_(-initrange, initrange)
                if module.bias is not None:
                    module.bias.data.zero_()
        for module in self.modification_head:
            if isinstance(module, nn.Linear):
                module.weight.data.uniform_(-initrange, initrange)
                if module.bias is not None:
                    module.bias.data.zero_()
    ...
    def forward(self, src_signal: Tensor, src_pad_mask: Tensor, src_target_mask: Tensor) -> Tuple[Tensor, Tensor]:
...
class RegressionHead(nn.Module):
    def __init__(self, d_model: int, d_out: int, lin_act: str, lin_depth: int, lin_dropout: float, seq_length: int):
        ...
        layer_list.append(nn.Linear(d_model, d_out))
        layer_list.append(self._get_activation_fn(lin_act))
