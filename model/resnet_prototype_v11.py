...
class MaskedSequential(nn.Sequential):
    def forward(self, input: Tensor, mask: Tensor) -> Tensor:
        for resnetmodule in self._modules.values():
            output = resnetmodule(input, mask)
...
class ResidualBlock(nn.Module):
    def __init__(self, kernel_size: int, in_channels: int, out_channels: int, stride: int = 1, padding = 'same',
        ...
        self.conv1 = nn.Conv1d(in_channels, out_channels*4, 1, stride, padding)
        self.conv2 = nn.Conv1d(out_channels*4, out_channels, kernel_size, stride, padding)
...
class ResNetModel(nn.Module):
    ...
    def __init__(self, n_layers_kmer: int, n_layers_sig: int, n_layers_merged: int, n_layers_final: int,
        ...
        self.start_module_kmer = [
                                  nn.Conv1d(4*kmer_size, d_kmer, 11, stride = 1, padding = 'same'),
                                  nn.AvgPool1d(5,5,0),
                                  nn.BatchNorm1d(d_kmer)]

        self.residual_module_kmer = []
        for idx in range(n_layers_kmer):
            self.residual_module_kmer.append(ResidualBlock(kernel_size, d_kmer + (d_kmer//4)* idx, d_kmer//4,
                                                           stride = 1, padding = 'same',
                                                           activation = act, dropout = dropout))


        self.start_module_signal = [ nn.Conv1d(3, d_signal, 11, stride = 1, padding = 'same'),
                                     nn.BatchNorm1d(d_signal)]

        self.residual_module_signal_1 = []
        for idx in range(n_layers_sig):
            self.residual_module_signal_1.append(ResidualBlock(kernel_size, d_signal + (d_signal//4)* idx, d_signal//4,
                                                               stride = 1, padding = 'same',
                                                               activation = act, dropout = dropout))

        self.end_module_signal = [act_func,
                                  nn.Conv1d(d_signal+ (d_signal//4)* n_layers_sig , d_signal, 5, stride = 5, padding = 'valid'),
                                  nn.BatchNorm1d(d_signal)]


        self.residual_module_signal_2 = []
        for idx in range(n_layers_sig):
            self.residual_module_signal_2.append(ResidualBlock(kernel_size, d_signal + (d_signal//4)* idx, d_signal//4,
                                                               stride = 1, padding = 'same',
                                                               activation = act, dropout = dropout))


        self.start_module_merged = [act_func,
                                    nn.Conv1d(d_signal + (d_signal//4) * n_layers_sig + d_kmer + (d_kmer//4) * n_layers_kmer ,
                                              d_merged, 1, stride = 1, padding = 'same'),
                                    nn.BatchNorm1d(d_merged),
                                    act_func,
                                    nn.Conv1d(d_merged, d_merged, 11, stride = 1, padding = 'same'),
                                    nn.BatchNorm1d(d_merged),]

        self.residual_module_merged = []

        for idx in range(n_layers_merged):
            self.residual_module_merged.append(ResidualBlock(kernel_size, d_merged + (d_merged//4)* idx, d_merged//4,
                                                               stride = 1, padding = 'same',
                                                               activation = act, dropout = dropout))


        self.regression_head_1 = [act_func,
                                  nn.Linear(d_merged + (d_merged//4)* n_layers_merged, d_merged),
                                  act_func]
        ...
        self.start_module_merged = nn.Sequential(*self.start_module_merged)
        ...
        self.start_module_merged = nn.SyncBatchNorm.convert_sync_batchnorm(self.start_module_merged)
    ...
    def forward(self, src_kmer: Tensor, src_signal: Tensor, src_bq: Tensor, src_move: Tensor, target_mask: Tensor) -> Tensor:
        ...
        merged_out = self.start_module_merged(merged_out) * pad_mask
