...
class TransformerModel(nn.Module):
    ...
    def __init__(self, d_model: int, n_heads: int, d_ff: int, n_layers: int,
        ...
        encoder_layer = transformer.TransformerEncoderLayer(d_model, n_heads, d_ff, dropout = encoder_dropout,
                                                            activation = t_act,batch_first=True)
        self.transformer_encoder = transformer.TransformerEncoder(encoder_layer, n_layers, enable_nested_tensor = False)
    ...
    def hook_fn(self, module, input, output):
        self.activation_tensor.append(module.activation_tensor)
        return None

    ## Set hook of each layer of the transformer encoder
    def set_hook(self):
