...
class TransformerModel(nn.Module):
    ...
    def forward(self, src_kmer: Tensor, src_signal: Tensor, src_seg_len: Tensor, src_bq: Tensor) -> Tensor:
        with torch.no_grad():
            ...
            src_kmer = ((((src_kmer - 65).clip(None,8)%5).unfold(1, self.kmer_size, 1) * (4**torch.arange(self.kmer_size, device = src_kmer.device, dtype = torch.int)).unsqueeze(0).unsqueeze(0)).sum(dim = -1) + 1 ).repeat_interleave(self.unit_size, dim=1)[:,self.kmer_size//2:-(self.kmer_size//2)]
            src_bq = src_bq.clip(0,self.max_bq).repeat_interleave(self.unit_size, dim=1)[:,self.kmer_size//2:-(self.kmer_size//2)]

        final_embedding = torch.stack([self.kmer_embedding(src_kmer), self.signal_embedding(src_signal), self.bq_embedding(src_bq), self.pos_encoding(src_kmer.shape[0])], dim = 0).sum(dim = 0)
