...
class TransformerModel(nn.Module):
    ...
    def forward(self, src_kmer: Tensor, src_signal: Tensor, src_bq: Tensor, src_move: Tensor,
        ...
        inverse_pad_mask = (src_pad_mask == 0)
        output = output * inverse_pad_mask

        inverse_pad_mask_sum = inverse_pad_mask.sum(dim = 1)
        target_mask_sum = target_mask.sum(dim = 1)

        background = output.sum(dim = 1)
        background = background / inverse_pad_mask_sum
        ...
        epsilon = 1e-6
        output = output / (background + epsilon)
