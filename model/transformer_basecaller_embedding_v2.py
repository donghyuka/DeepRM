...
class TransformerModel(nn.Module):
    ...
    def __init__(self, d_model: int, n_heads: int, d_ff: int,
        ...
        self.weight_arr = np.tile(np.arange(4), self.kmer_size) * (np.repeat(4**np.arange(self.kmer_size), 4))
        self.weight_arr = torch.tensor(self.weight_arr, dtype=torch.float).unsqueeze(0).unsqueeze(-1)
    ...
    def forward(self, src_kmer: Tensor, src_signal: Tensor, src_pad_mask: Tensor, target_mask: Tensor) -> Tensor:
        ...
        src_pad_mask = torch.nn.functional.max_pool1d(src_pad_mask.unsqueeze(1).float(), kernel_size=5, stride=5).squeeze(1).bool()
        src_kmer = torch.nn.functional.max_pool1d(src_kmer.flatten(start_dim=-2).permute(0, 2, 1).float(), kernel_size=5, stride=5)
        weight_arr = self.weight_arr.to(src_kmer.device)
        src_kmer = src_kmer * weight_arr
        src_kmer = src_kmer.sum(dim = 1, keepdim = False) + 1
        src_kmer = src_kmer * (src_pad_mask==0)
        src_kmer = src_kmer.long()
        target_mask = torch.nn.functional.max_pool1d(target_mask.unsqueeze(1).float(), kernel_size=5, stride=5).squeeze(1)

        print(src_kmer[0].detach().cpu().numpy())
        print(src_signal[0].detach().cpu().numpy())
        print(target_mask[0].detach().cpu().numpy())
        print(src_pad_mask[0].detach().cpu().numpy())
