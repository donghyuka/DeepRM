...
class TransformerModel(nn.Module):
    ...
    def forward(self, src_kmer: Tensor, src_signal: Tensor, src_seg_len: Tensor, src_dwell_bq: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        final_embedding = self.transformer_encoder(src=final_embedding, mask = None, src_key_padding_mask = src_pad_mask)

        ## apply regression head to each token:
        output = self.regression_head(final_embedding)
        ...
        final_embedding = [final_embedding[i,(target_prev_end[i], target_end[i], target_next_end[i])].cpu().detach().numpy() for i in range(final_embedding.size(0))]
