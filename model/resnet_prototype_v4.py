...
class ResNetModel(nn.Module):
    ...
    def __init__(self, n_layers_kmer: int, n_layers_sig: int, n_layers_merged: int, n_layers_final: int,
        ...
        self.start_module_signal = [nn.Conv1d(3, d_kmer, 5, stride = 5, padding = 'valid'),
                                    nn.BatchNorm1d(d_kmer), act_func]
        ...
        for _ in range(n_layers_final-4):
            self.regression_head.append(nn.Linear(current_dim, current_dim))
    ...
    def forward(self, src_kmer: Tensor, src_signal: Tensor, src_bq: Tensor, src_move: Tensor, target_mask: Tensor) -> Tensor:
        ...
        signal_out = self.residual_module_signal(signal_out, pad_mask)
        output = torch.cat([kmer_out, signal_out], dim = 1)
