...
class TransformerModel(nn.Module):
    ...
    def __init__(self, d_model: int, n_heads: int, d_ff: int,
        ...
        self.regression_head_2 = RegressionHead2(d_model, lin_act, lin_depth, lin_dropout, seq_len)
    ...
    def freeze_and_reinit(self, initrange = 0.1):
        ...
        self.regression_head_2.init_weights(initrange)
        for param in self.regression_head_2.parameters():
    ...
    def forward(self, src_kmer: Tensor, src_signal: Tensor, src_seg_len: Tensor, src_bq: Tensor, src_mfe = Tensor) -> Tensor:
        ...
        target_mask_sum = target_mask.sum(dim = 1, keepdim = True)
        output = output * target_mask
        output = output.sum(dim = 1, keepdim = True)
        output = output / target_mask_sum

        output = torch.cat([output, src_mfe.unsqueeze(-1)], dim = -1)
        output = self.regression_head_2(output).flatten()
...
class RegressionHead2(nn.Module):
    def __init__(self, d_model: int, lin_act: str, lin_depth: int, lin_dropout: float, seq_length: int):
        super().__init__()
        layer_list=  []
        layer_list.append(nn.Linear(2, 16))
        layer_list.append(get_activation_fn(lin_act))
        layer_list.append(nn.Linear(16, 1))
        self.lin_layers = nn.Sequential(*layer_list)
