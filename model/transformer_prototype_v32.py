...
class TransformerModel(nn.Module):
    ...
    def forward(self, src_kmer: Tensor, src_signal: Tensor, src_bq: Tensor, src_move: Tensor,
        ...
        div_term = torch.exp(torch.arange(0, self.d_model // 2, 2) * (-math.log(10000.0) / self.d_model // 2)).to(src_bq.device)
        ...
        pos_encoding = torch.concat([pos_encoding, mv_embedding], dim = -1)
        output = torch.stack([kmer_embedding, signal_embedding, pos_encoding], dim = 0).sum(dim = 0)
        ...
        output = torch.add(output, bq_embedding)
