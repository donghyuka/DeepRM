...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x: Tensor, pad_mask: Tensor) -> Tensor:
        ...
        out += self.shortcut(x)
        out = self.activation(out) * pad_mask
        return out
...
class ResNetModel(nn.Module):
    ...
    def __init__(self, n_layers_kmer: int, n_layers_sig: int, n_layers_merged: int, n_layers_final: int,
        ...
        self.start_module_signal = [nn.Conv1d(3, d_signal, kernel_size, stride = 1, padding = 'same'),
        ...
        self.residual_module_merged_1 = [ResidualBlock(kernel_size, d_signal+d_kmer, d_merged, stride = 1,
                                                     padding = 'same', activation = act, dropout = dropout),]

        for _ in range(n_layers_merged-1):
            self.residual_module_merged_1.append(ResidualBlock(kernel_size, d_merged, d_merged, stride = 1,
                                                               padding = 'same', activation = act, dropout = dropout))

        self.residual_module_merged_2 = []

        for _ in range(n_layers_merged):
            self.residual_module_merged_2.append(ResidualBlock(kernel_size, d_merged, d_merged, stride = 1,
                                                             padding = 'same', activation = act, dropout = dropout))

        self.residual_module_merged_3 = []

        for _ in range(n_layers_merged):
            self.residual_module_merged_3.append(ResidualBlock(kernel_size, d_merged, d_merged, stride = 1,
                                                             padding = 'same', activation = act, dropout = dropout))

        self.residual_module_merged_4 = []

        for _ in range(n_layers_merged):
            self.residual_module_merged_4.append(ResidualBlock(kernel_size, d_merged, d_merged, stride = 1,
                                                             padding = 'same', activation = act, dropout = dropout))
        ...
        self.residual_module_signal =   MaskedSequential(*self.residual_module_signal)
        self.residual_module_merged_1 = MaskedSequential(*self.residual_module_merged_1)
        self.residual_module_merged_2 = MaskedSequential(*self.residual_module_merged_2)
        self.residual_module_merged_3 = MaskedSequential(*self.residual_module_merged_3)
        self.residual_module_merged_4 = MaskedSequential(*self.residual_module_merged_4)
        ...
        self.residual_module_signal = nn.SyncBatchNorm.convert_sync_batchnorm(self.residual_module_signal)
        self.residual_module_merged_1 = nn.SyncBatchNorm.convert_sync_batchnorm(self.residual_module_merged_1)
        self.residual_module_merged_2 = nn.SyncBatchNorm.convert_sync_batchnorm(self.residual_module_merged_2)
        self.residual_module_merged_3 = nn.SyncBatchNorm.convert_sync_batchnorm(self.residual_module_merged_3)
        self.residual_module_merged_4 = nn.SyncBatchNorm.convert_sync_batchnorm(self.residual_module_merged_4)
    ...
    def forward(self, src_kmer: Tensor, src_signal: Tensor, src_bq: Tensor, src_move: Tensor, target_mask: Tensor) -> Tensor:
        ...
        kmer_out = self.residual_module_kmer(src_kmer, pad_mask)
        signal_out = self.residual_module_signal(src_signal, pad_mask)
        merged_out = torch.cat([kmer_out, signal_out], dim = 1) * pad_mask
        merged_out = self.residual_module_merged_1(merged_out, pad_mask)
        merged_out = torch.nn.AvgPool1d(2,2,0)(merged_out)
        pad_mask = torch.nn.AvgPool1d(2,2,0)(pad_mask)
        merged_out = self.residual_module_merged_2(merged_out, pad_mask)
        pad_mask = torch.nn.AvgPool1d(2,2,0)(pad_mask)
        merged_out = torch.nn.AvgPool1d(2,2,0)(merged_out)
        merged_out = self.residual_module_merged_3(merged_out, pad_mask)
        pad_mask = torch.nn.AvgPool1d(2,2,0)(pad_mask)
        merged_out = torch.nn.AvgPool1d(2,2,0)(merged_out)
        merged_out = self.residual_module_merged_4(merged_out, pad_mask)

        pad_mask = pad_mask.squeeze(1).unsqueeze(-1)
        output = merged_out.permute(0, 2, 1)
        output = self.regression_head_1(output) * pad_mask
